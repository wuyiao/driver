...
#ifdef ARM64
uapi AvRet AvUapiAllocateMemory(pin uint32 bytes, pout /*uint32*/uint64 *bufferAddress)
{
    AvRet ret = AvOk;
    static uint32 currentPoint = 0;
    uint32 wordbytes = bytes & 0xfffffffc;
	...
	static int count = 0;
	if (count == 1) {
		printk("%s currentPoint : %d &AvGlobalStaticBuffer[currentPoint] : %x !!!!!!!!\n", __func__, currentPoint, (uint64)(&AvGlobalStaticBuffer[currentPoint]));
	}
	count++;
#endif
    if ((currentPoint + bytes) < AvGlobalStaticBufferSize)
    {
        *bufferAddress = /*(uint32)*/(uint64)(&AvGlobalStaticBuffer[currentPoint]);
        if(bytes != wordbytes)
            bytes = wordbytes + 4;
        currentPoint = currentPoint + bytes;
    }
