#include "gsv2k11.h"
//#include "gsv2k11_lib.h"
#include "GSV2K11_lib.h"
#include "hal.h"
#include "uapi.h"
#include "uapi_function_mapper.h"
#include "gsv2k11_tables.h"
...
static Resource InternalResource;

#define FindCp1Mode(port)      (InternalResource.Cp1CoreOccupied)
#if AvEdidStoredInRam
#define TxBEdidRam(port)       (InternalResource.TxBEdidRam)
#endif

#define Gsv2k11SetVideoPacketFlag(BitName) \
...
#define Gsv2k11SetAudioPacketFlag(BitName) \
...
#define Gsv2k11IntptSt(func, value) \
            GSV2K11_INT_get_RX1_##func##_INT_ST(port,         &value);

#define Gsv2k11IntptRaw(func) \
            GSV2K11_INT_get_RX1_##func##_RAW_ST(port,         &Value);

#define Gsv2k11IntptClr(func) \
            GSV2K11_INT_set_RX1_##func##_CLEAR(port,         1);

/* local functions */
void Gsv2k11GetRx5VStatus(pin AvPort *port);
void Gsv2k11EnableRxHpa(pin AvPort *port);
void Gsv2k11DisableRxHpa(pin AvPort *port);
void Gsv2k11ToggleTxHpd(pin AvPort *port);
void Gsv2k11ToggleTmdsOut(AvPort *port, uint8 Enable);
void Gsv2k11RxManualEQUpdate(AvPort *port, uint8 DefaultEQEnable);
void Gsv2k11SetTxHdcpVersion(pin AvPort *port, HdcpTxSupportType HdmiStyle, uint8 ForceUpdate);
void Gsv2k11UpdateRxCdrBandWidth(pin AvPort *port);
uint8 Gsv2k11TxScdcAction(pin AvPort *port, uint8 WriteEnable, uint8 DevAddr, uint8 RegAddr, uint8 Value);
uint8 Gsv2k11TxDDCError(pin AvPort *port);
void Gsv2k11ManualBksvRead(pin AvPort *port);
AvPort *Gsv2k11FindHdmiRxFront(pin AvPort *port);
AvVideoCs Gsv2k11ColorCsMapping(AvVideoCs InputCs);
void Gsv2k11ManualCpParameter(AvPort *port);
void Gsv2k11TxPllUnlockClear(AvPort *port);
uint8 Gsv2k11TweakCrystalFreq(AvPort *port, uint32 value32);
void Gsv2k11MpllProtect(AvPort *port);
void Gsv2k11RpllProtect(AvPort *port);
void Gsv2k11ResetTxFifo(pin AvPort *port);
void Gsv2k11ToggleDpllFreq(pin AvPort *port, uint8 index, uint8 Integer, uint8 *Fraction);
void Gsv2k11CpCscManage(pin AvPort *port, AvVideoCs VarInCs, AvVideoCs VarOutCs);

/**
 * @brief  device init function
 * @return AvOk if success
 */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiInitDevice(pin AvDevice *device))
{
    ...
    for(k=0; Gsv2k11EqTable[k]!=0xFF; k=k+2)
        AvHalI2cWriteField8(GSV2K11_RXLN_MAP_ADDR(port),Gsv2k11EqTable[k],0xFF,0,Gsv2k11EqTable[k+1]);
 ...
 * @note
 */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiRxPortInit(pin AvPort *port))
{
    /* only RxPort occupies RxCore*/
    port->core.HdmiCore = 0;
    /* only routing for RxPort */
    GSV2K11_PRIM_set_RX1_CORE_SEL(port, 0);

    Gsv2k11DisableRxHpa(port);
    return AvOk;
}
 ...
 * @note
 */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiTxPortInit(pin AvPort *port))
{
    AvRet ret = AvOk;
#if AvEnableCecFeature /* CEC Related */
    /* Gsv2k11 has 1 input port */
 ...
 * @note
 */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiTxEnableCore(pin AvPort *port))
{
    AvRet ret = AvOk;

    if(port->index == 5)
    {
        GSV2K11_PRIM_set_TXA_PWR_DN(port, 0);
        GSV2K11_PRIM_set_TXB_PWR_DN(port, 0);
    }
    else
        ret = AvError;
 ...
 * @note
 */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiTxDisableCore(pin AvPort *port))
{
    AvRet ret = AvOk;
    uint32 MainMapAddress   = GSV2K11_TXDIG_MAP_ADDR(port);
    uint32 PhyMapAddress    = GSV2K11_TXPHY_MAP_ADDR(port);

    /* Disable HDCP */
    AvUapiTxDecryptSink(port);
    /* Set HDCP Version Before Reset Tx */
    Gsv2k11SetTxHdcpVersion(port, port->content.hdcptx->HdmiStyle, 0);
    /* Disable Tx TMDS Clock Driver, only enable after video is fully stable */
    Gsv2k11ToggleTmdsOut(port, 0);
    ...
    if(port->core.HdmiCore == 0)
    {
        ...
        GSV2K11_TXPKT_set_TX_PKT_UPDATE(port, 0);
    }
    /* HDCP 2.2 Stop on Error */
    GSV2K11_TX2P2_set_TX_HDCP2P2_OP_WHEN_ERROR(port, 1);
    ...
    GSV2K11_TX2P2_set_REVOCATION_CHECK_PASS(port, 1);
    /* HDCP 2.2 Stop on reset */
    GSV2K11_TX2P2_set_TX_HDCP2P2_FSM_DISABLE(port, 1);
    /* default DDC Speed */
    GSV2K11_TX2P2_set_HDCP2P2_MASTER_CLK_DIV(port, 0x14A);
    /* HDCP 2.2 i2c_clear_int_rxid_ready=1 */
    Gsv2k11AvUapiTxClearRxidReady(port);
#endif
    ...
    {
        Gsv2k11AvUapiTxCecInit(port);
        Gsv2k11AvUapiTxCecSetLogicalAddr(port);
    }
#endif
...
{
    /* reset dc fifo */
    GSV2K11_TXDIG_set_DCFIFO_RESET(port, 1);
    GSV2K11_TXDIG_set_DCFIFO_RESET(port, 0);
    /* reset audio fifo */
    GSV2K11_TXDIG_set_TX_AUDIOFIFO_RESET(port, 1);
    GSV2K11_TXDIG_set_TX_AUDIOFIFO_RESET(port, 0);
    /* reset phy */
    GSV2K11_TXPHY_set_TMDS_SRST_MAN(port, 0);
    GSV2K11_TXPHY_set_TMDS_SRST_MAN(port, 1);
}
 ...
 * @note
 */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiResetPort(pin AvPort *port))
{
    AvRet ret = AvOk;
    ...
    if(port->type == HdmiRx)
    {
        ...
        Gsv2k11RxManualEQUpdate(port, 1);
    }
    else if(port->type == HdmiTx)
    {
        switch(port->index)
        {
            case 5: /* tx port 1 */
                /* Reset Tx */
                Gsv2k11AvUapiTxDisableCore(port);
                Gsv2k11AvUapiTxEnableCore(port);
                /* Reset Edid Segment Ptr */
                GSV2K11_TXPHY_set_TX_DDC_READ_EDID_RAM_PART(port, 0);
                /* Edid will be reread 15 times if failed */
                GSV2K11_TXPHY_set_TX_EDID_TRY_TIMES(port,15);
                break;
            default:
                break;
        }
 ...
 * @note
 */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiEnablePort(pin AvPort *port))
{
    return AvOk;
}
 ...
 * @return AvOk: success
 */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiRxEnableFreeRun(pin AvPort *port, bool enable))
{
    return AvOk;
}
 ...
 * @return AvOk: success
 */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiRxGet5VStatus(pin AvPort *port))
{
    AvRet ret = AvOk;

    Gsv2k11GetRx5VStatus(port);
 ...
 * @return AvOk: success
 */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiRxGetStatus(pin AvPort *port))
{
    uint8 value;
    uint8 temp;
    uint32 value32;
    ...
    {
        GSV2K11_PLL_get_RB_RXA_PLL_VCO_FREQ(port, &value32);
        GSV2K11_PLL_get_RB_RXA_PLL_SER_DIV(port, &value);
        temp = (1<<value) * 5;
        ...
        Gsv2k11AvUapiRxGetVideoLock(port);
        /* there is a mapped core existed */
        GSV2K11_RXDIG_get_RB_RX_HDMI_MODE(port, &(port->content.rx->HdmiMode));
        GSV2K11_RXDIG_get_RB_RX_PIXEL_REPETITION(port, &(port->content.video->PixelRepeatValue));
        GSV2K11_RXDIG_get_RB_RX_DEEP_COLOR_MODE(port, &DeepColor);
        ...
        if(port->content.rx->Lock.PllLock == 1)
        {
               ...
               (port->content.rx->Lock.VSyncLock == 1))
            {
                if(port->content.rx->IsInputStable == 0)
                {
                    ...
                    GSV2K11_RXAUD_set_CLEAR_AUDIO_HB_CONTENT(port, 1);
                    GSV2K11_RXAUD_set_CLEAR_AUDIO_HB_CONTENT(port, 0);
                }
            ...
            {
                port->content.rx->IsInputStable = 0;
                Gsv2k11RxManualEQUpdate(port, 0);
            }
        }

        if(port->content.rx->IsInputStable == 1)
        {
            GSV2K11_RXAUD_get_ACR_PKT_DET(port, &value);
            if(value == 1)
            {
                /* only set APLL lock process after N/CTS is stable */
                GSV2K11_RXDIG_get_RX_APLL_MAN_START_CALC(port, &value);
                if(value == 1)
                {
                    GSV2K11_RXDIG_set_N_CTS_STABLE_DET_START(port, 0);
                    GSV2K11_RXDIG_set_RX_APLL_MAN_START_CALC(port, 0);
                    GSV2K11_RXAUD_set_CLEAR_AUDIO_HB_CONTENT(port, 1);
                    GSV2K11_RXAUD_set_CLEAR_AUDIO_HB_CONTENT(port, 0);
                ...
                {
                    GSV2K11_RXAUD_get_RX_AUDIO_PLL_LOCKED(port, &port->content.rx->Lock.AudioLock);
                    if(port->content.rx->Lock.AudioLock)
                    {
                        /* Get internal actual audio mute status */
                        Gsv2k11AvUapiRxGetAudioInternalMute(port);
                        /* Possible Incorrect Garbage Audio Type Recovery */
                        AvHalI2cReadField8(GSV2K11_RXAUD_MAP_ADDR(port), 0x17, 0x1E, 0, &value);
                        if(value != 0)
                        {
                            GSV2K11_RXAUD_set_CLEAR_AUDIO_HB_CONTENT(port, 1);
                            GSV2K11_RXAUD_set_CLEAR_AUDIO_HB_CONTENT(port, 0);
                        ...
                        /* Set DSD Audio Mode */
                        GSV2K11_RXAUD_get_MAN_TDM_ENABLE(port, &value);
                        if((port->content.audio->AudType == AV_AUD_TYPE_DSD) ||
                           (port->content.audio->AudType == AV_AUD_TYPE_DST))
                            ...
                            temp = 1;
                        if(temp != value)
                            GSV2K11_RXAUD_set_MAN_TDM_ENABLE(port, temp);
                        /* Channel Number */
                        GSV2K11_RXDIG_get_RB_RX_AUDIO_LAYOUT(port, &port->content.audio->Layout);
                        if(port->content.audio->Layout == 0)
                        {
                            port->content.audio->ChanNum    = 2;
                            GSV2K11_RXAUD_get_RX_AUDIO_PIN_NUM(port, &value);
                            if((port->content.audio->AudType == AV_AUD_TYPE_DSD) ||
                               (port->content.audio->AudType == AV_AUD_TYPE_DST))
                                temp = 1;
                            else
                                temp = 0;
                            if(value != temp)
                                GSV2K11_RXAUD_set_RX_AUDIO_PIN_NUM(port, temp);
                        }
                        else
                        {
                            GSV2K11_RXAUD_get_AUD_IF_PKT_DET(port, &value);
                            if(value == 1)
                            {
                                AvHalI2cReadField8(GSV2K11_RXINFO_MAP_ADDR(port),0x1D,0xFF,0,&value);
                                port->content.audio->ChanNum = (value & 0x07) + 1;
                                /* Check Audio Channel Using Logic Rather than Packet */
                                GSV2K11_RXAUD_get_NUM_CH(port, &value);
                                temp = (value-1)>>1;
    ...
    {
        AvHalI2cReadField8(GSV2K11_RXAUD_MAP_ADDR(port), 0x19, 0xFF, 0, &temp);
        /* AVI_INFO_FRAME */
        value = (temp>>6) & 0x01;
        Gsv2k11SetVideoPacketFlag(AV_BIT_AV_INFO_FRAME);
        /* AUDIO_INFO */
        value = (temp>>7) & 0x01;
        Gsv2k11SetAudioPacketFlag(AV_BIT_AUDIO_INFO_FRAME);
        AvHalI2cReadField8(GSV2K11_RXAUD_MAP_ADDR(port), 0x18, 0xFF, 0, &temp);
        /* SPD_INFO */
        value = (temp>>0) & 0x01;
        Gsv2k11SetVideoPacketFlag(AV_BIT_SPD_PACKET);
        /* MS_INFO */
        value = (temp>>1) & 0x01;
        Gsv2k11SetVideoPacketFlag(AV_BIT_MPEG_PACKET);
        /* VS_INFO */
        value = (temp>>2) & 0x01;
        Gsv2k11SetVideoPacketFlag(AV_BIT_VS_PACKET);
        /* ACP_PCKT */
        value = (temp>>3) & 0x01;
        Gsv2k11SetAudioPacketFlag(AV_BIT_ACP_PACKET);
        /* ISRC1_PCKT */
        value = (temp>>4) & 0x01;
        Gsv2k11SetVideoPacketFlag(AV_BIT_ISRC1_PACKET);
        /* ISRC2_PCKT */
        value = (temp>>5) & 0x01;
        Gsv2k11SetVideoPacketFlag(AV_BIT_ISRC2_PACKET);
        /* Gamut */
        value = (temp>>6) & 0x01;
        Gsv2k11SetVideoPacketFlag(AV_BIT_GMD_PACKET);
        /* ACR Packet */
        value = (temp>>7) & 0x01;
        Gsv2k11SetAudioPacketFlag(AV_BIT_ACR_PACKET);
        AvHalI2cReadField8(GSV2K11_RXAUD_MAP_ADDR(port), 0x17, 0xFF, 0, &temp);
        /* GC */
        value = (temp>>0) & 0x01;
        Gsv2k11SetVideoPacketFlag(AV_BIT_GC_PACKET);
        /* HDR_INFO */
        value = (temp>>6) & 0x01;
        Gsv2k11SetVideoPacketFlag(AV_BIT_HDR_PACKET);
        /* EMP Packet */
        value = (temp>>7) & 0x01;
        Gsv2k11SetVideoPacketFlag(AV_BIT_EMP_PACKET);
        /* Channel Status */
        GSV2K11_RXAUD_get_CSDATA_VALID(port, &value);
        Gsv2k11SetAudioPacketFlag(AV_BIT_AUDIO_CHANNEL_STATUS);
        /* AvMute */
        GSV2K11_INT_get_RX1_AVMUTE_RAW_ST(port, &(port->content.video->Mute.AvMute));
        /* Hdmi Mode */
        GSV2K11_INT_get_RX1_HDMI_MODE_RAW_ST(port, &(port->content.rx->HdmiMode));
 ...
 * @return AvOk: success
 */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiRxSetHpdUp(pin AvPort *port))
{
    AvRet ret = AvOk;

    Gsv2k11EnableRxHpa(port);
 ...
 * @return AvOk: success
 */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiRxSetHpdDown(pin AvPort *port))
{
    AvRet ret = AvOk;
 ...
 * @return AvOk: success
 */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiTxMuteTmds(pin AvPort *port, bool mute))
{
    uint8 Enable = 0;
    if(mute == 0)
        Enable = 1;
    Gsv2k11ToggleTmdsOut(port, Enable);
 ...
 * @return AvOk: success
 */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiTxGetStatus(pin AvPort *port))
{
    AvRet ret = AvOk;
    uint8 value = 0;
    ...
    FromPort = (AvPort*)(Gsv2k11FindHdmiRxFront(port));

    if((port->content.tx->Hpd != AV_HPD_FORCE_LOW) &&
       (port->content.tx->Hpd != AV_HPD_FORCE_HIGH))
    {
        GSV2K11_TXPHY_get_HPD_RAW_STATE(port, &value);
        /* Tx Detection Power Down */
        switch(port->index)
        {
            case 5:
                GSV2K11_INT_get_TXB_HPD_INTR_INT_ST(port, &NewValue);
                if(NewValue == 1)
                {
                    GSV2K11_INT_set_TXB_HPD_INTR_CLEAR(port, 1);
                    value = 0;
                }
                if((value == 1) && (port->content.tx->Hpd == AV_HPD_LOW))
                {
                    GSV2K11_PRIM_set_TXB_TERM_DET_PWR_DN(port, 0);
                    GSV2K11_TXPHY_get_MSEN_RAW_STATE(port, &value);
                    GSV2K11_PRIM_set_TXB_TERM_DET_PWR_DN(port, 1);
                }
                break;
        }
    ...
    if((port->content.tx->Hpd != AV_HPD_LOW) && (FromPort != NULL))
    {
        if(port->core.HdmiCore == 0)
        {
            ...
            {
                GSV2K11_PLL_get_RB_TXA_PLL_LOCK_CAPTURED(port, &value);
                if(value == 0)
                    Gsv2k11TxPllUnlockClear(port);
            }
            else
            {
                GSV2K11_PLL_get_RB_TXA_PLL_LOCK(port, &value);
                if((value == 0) && (FromPort->content.rx->Lock.PllLock == 1))
                    Gsv2k11RpllProtect(port);
            }
            if(value == 1)
            {
    ...
    {
        Gsv2k11ToggleTmdsOut(port, 0); /* Disable TMDS Out */
    ...
    if(LocalPllLock)
    {
        ...
        if(port->core.HdmiCore == 0)
        {
            GSV2K11_PLL_get_RB_TXA_PLL_REF_CLK_FREQ(port, &value32);
            /* Reread Ref Clock Frequency */
            if(Gsv2k11TweakCrystalFreq(port, value32) == 1)
                GSV2K11_PLL_get_RB_TXA_PLL_REF_CLK_FREQ(port, &value32);
            value32 = value32>>8;
            GSV2K11_PLL_get_RB_TXA_PLL_PRE_DIV(port, &value);
            value32 = value32/value;
            GSV2K11_PLL_get_RB_TXA_PLL_POST_DIV(port, &temp);
            value32 = value32 * temp;
            if(port->index == 5)
                GSV2K11_PLL_get_RB_TXB_PLL_SER_DIV(port, &value);
            else
                value = 0;
            temp = (1<<value) * 5;
            value32 = value32/temp;
        ...
        if(Freq3G4Plus == 1)
            ...
            NewValue = 0;
        /* 3.1 Phy Mode Change */
        GSV2K11_TXPHY_get_TX_HDMI2_MODE_MAN(port, &value);
        if(value != NewValue)
        {
            GSV2K11_TXPHY_set_TX_HDMI2_MODE_MAN_EN(port, 1);
            GSV2K11_TXPHY_set_TX_HDMI2_MODE_MAN(port, NewValue);
        }
        /* 3.2 Scramble Change */
        GSV2K11_TXPHY_get_TX_SCR_EN_MAN(port, &value);
        if(value != NewValue)
        {
            GSV2K11_TXPHY_set_TX_SCR_EN_MAN(port, NewValue);
            GSV2K11_TXPHY_set_TX_SCR_EN_MAN_EN(port, 1);
        }
        /* 3.3 SCDC Change */
        GSV2K11_TXPHY_get_EDID_ANALYZE_DONE(port, &value);
        if(value == 0)
            GSV2K11_TXPHY_set_EDID_ANALYZE_DONE(port, 1);
    }

    /* Find Final Lock Status, only pass the test to have Tx Pll Lock */
    if(port->content.tx->InfoReady == 1)
    {
        Gsv2k11ResetTxFifo(port);
        AvUapiTxSetHdmiModeSupport(port);
    }
    if(port->content.tx->InfoReady == TxVideoManageThreshold)
    {
        ...
        if(port->content.video->info.TmdsFreq > 340)
        {
            ...
            AvHalI2cWriteField8(GSV2K11_TXPHY_MAP_ADDR(port),0xD1,0xFF,0,0xFE);
            AvHalI2cWriteField8(GSV2K11_TXPHY_MAP_ADDR(port),0xD3,0xFF,0,0x5F);
            AvHalI2cWriteField8(GSV2K11_TXPHY_MAP_ADDR(port),0xD4,0xFF,0,0x47);
            AvHalI2cWriteField8(GSV2K11_TXPHY_MAP_ADDR(port),0xD5,0xFF,0,0xBE);
            AvHalI2cWriteField8(GSV2K11_TXPHY_MAP_ADDR(port),0xD6,0xFF,0,0x1F);
        #else
            /* Slew Rate: 0x14/0x34/0x54/0x74 */
            AvHalI2cWriteField8(GSV2K11_TXPHY_MAP_ADDR(port),0xD4,0xFF,0,0x54);//Tx Phy Characater, Preemp = 101
        #endif
        }
        else
        {
            ...
            {
                AvHalI2cWriteField8(GSV2K11_TXPHY_MAP_ADDR(port),0xD1,0xFF,0,0xFE);
                AvHalI2cWriteField8(GSV2K11_TXPHY_MAP_ADDR(port),0xD3,0xFF,0,0x0F);
                AvHalI2cWriteField8(GSV2K11_TXPHY_MAP_ADDR(port),0xD4,0xFF,0,0x47);
                AvHalI2cWriteField8(GSV2K11_TXPHY_MAP_ADDR(port),0xD5,0xFF,0,0xB6);
                AvHalI2cWriteField8(GSV2K11_TXPHY_MAP_ADDR(port),0xD6,0xFF,0,0x03);
            }
            else
            {
                AvHalI2cWriteField8(GSV2K11_TXPHY_MAP_ADDR(port),0xD1,0xFF,0,0xF0);
                AvHalI2cWriteField8(GSV2K11_TXPHY_MAP_ADDR(port),0xD3,0xFF,0,0xAF);
                AvHalI2cWriteField8(GSV2K11_TXPHY_MAP_ADDR(port),0xD4,0xFF,0,0x40);
                AvHalI2cWriteField8(GSV2K11_TXPHY_MAP_ADDR(port),0xD5,0xFF,0,0x0C);
                AvHalI2cWriteField8(GSV2K11_TXPHY_MAP_ADDR(port),0xD6,0xFF,0,0x03);
            }
        #else
            /* Slew Rate: 0x10/0x30/0x50/0x70 */
            AvHalI2cWriteField8(GSV2K11_TXPHY_MAP_ADDR(port),0xD4,0xFF,0,0x50);//Tx Phy Characater, Preemp = 100
        #endif
        }
        /* 7.1 Protect for HDCP 2.2 Restart, disable EDID and SCDC in advance */
        GSV2K11_TX2P2_set_TX_HDCP2P2_FSM_DISABLE(port, 1);
        GSV2K11_TX2P2_set_TX_HDCP2P2_FSM_DISABLE(port, 0);
        /* 7.2 Manual Write SCDC Scramble and Clock Divider */
        GSV2K11_TXPHY_set_RX_SCDC_PRESENT(port, 0); /* no disturb on sink PLL lock from DDC */
        if((port->content.tx->EdidSupportFeature & AV_BIT_FEAT_SCDC) != 0x0)
        {
            ...
            if(NewValue == 1) /* Set SCDC */
                ...
                NewValue = 0x00;
            Gsv2k11TxScdcAction(port, 1, 0xA8, 0x01, 0x01);
            Gsv2k11TxScdcAction(port, 1, 0xA8, 0x20, NewValue);
            Gsv2k11ToggleTmdsOut(port, 1); /* Enable TMDS Out */
        }
        else
        {
            GSV2K11_TXPHY_get_TX_TMDS_CLK_DRIVER_ENABLE(port, &value);
            if(value == 0)
                Gsv2k11ToggleTmdsOut(port, 1);
        }
    ...
    if((port->content.tx->EdidSupportFeature & AV_BIT_FEAT_SCDC) != 0x0)
    {
        ...
        {
            GSV2K11_TXPHY_get_TX_SCDC_EN(port, &value);
            if(value == 0)
            {
                GSV2K11_TXPHY_set_TX_SCDC_EN(port, 1);
                GSV2K11_TXPHY_set_TX_RR_CAPABLE(port, 0); /* Auto CED readback */
                GSV2K11_TXPHY_set_RX_SCDC_PRESENT(port, 1); /* SCDC status check enable */
            }
        }
        else if(port->content.tx->InfoReady > TxHdcpManageThreshold)
        {
            GSV2K11_TXPHY_get_RX_SCDC_UPDATE_0(port, &value);
            if(value != 0x00)
            {
                value = value & 0x03; /* set task */
                GSV2K11_TXPHY_set_TX_CLR_UPDATE_FLAG(port, value);
                value = value | 0x80; /* task action */
                AvHalI2cWriteField8(GSV2K11_TXPHY_MAP_ADDR(port),0x07,0xFF,0,value);
        ...
        if(port->core.HdmiCore == 0)
        {
            ...
            if(port->content.tx->InfoReady == 1)
            {
                   ...
                   ((port->content.video->AvailableVideoPackets & AV_BIT_AV_INFO_FRAME) == 0))
                {
                    ...
                    Packet[1] = 0x02;
                    Packet[2] = 0x0D;
                    Gsv2k11_TxSendAVInfoFrame(port, Packet, 1);
                }
            }
            if(port->content.tx->InfoReady >= TxVideoManageThreshold)
            {
                /* Video Black Mute */
                GSV2K11_TXDIG_get_TX_BLACK_VIDEO_EN(port, &value);
                if(value != port->content.video->Mute.BlkMute)
                    GSV2K11_TXDIG_set_TX_BLACK_VIDEO_EN(port, port->content.video->Mute.BlkMute);
                /* Audio Mute */
                GSV2K11_TXDIG_get_TX_AUDIO_MUTE(port, &value);
                ...
                if(port->content.audio->AudioMute == 1)
                    ...
                    NewValue = 1;
                if(value != NewValue)
                    GSV2K11_TXDIG_set_TX_AUDIO_MUTE(port, NewValue);
            }
        }
    }
    else
    ...
    /* Edid Status Update */
    GSV2K11_TXPHY_get_TX_EDID_READY_RB(port,    &value);
    ...
    else if((value == 1) && (port->content.tx->EdidReadSuccess != AV_EDID_UPDATED))
    {
        ...
        GSV2K11_TXPHY_get_TX_DDC_READ_EDID_RAM_PART(port, &NewValue);
        if(NewValue == 0)
        {
            /* HF-EEODB */
            AvHalI2cReadMultiField(GSV2K11_TXEDID_MAP_ADDR(port), 0x84, 3, Packet);
            if((Packet[0] == 0xE2) && (Packet[1] == 0x78))
                NewValue = Packet[2];
            else
                AvHalI2cReadField8(GSV2K11_TXEDID_MAP_ADDR(port), 0x7E, 0xFF, 0, &NewValue);
            if((NewValue > 1) && (NewValue < 0x80))
            {
                AvMemset(TxBEdidRam(port), 0, AvEdidMaxSize);
                AvHalI2cReadMultiField(GSV2K11_TXEDID_MAP_ADDR(port), 0, 256, TxBEdidRam(port));
                AvUapiOutputDebugMessage("Port%d: Read Segment 0",port->index);
                GSV2K11_TXPHY_set_TX_DDC_READ_EDID_RAM_PART(port, 1);
                GSV2K11_TXPHY_set_TX_EDID_RE_READ(port, 0);
                GSV2K11_TXPHY_set_TX_EDID_RE_READ(port, 1);
        ...
        else
        {
            ...
            if(NewValue < value)
            {
                if(NewValue >= (AvEdidMaxSize/256))
                    ...
                    value32 = NewValue;
                if(value32 != 0)
                    AvHalI2cReadMultiField(GSV2K11_TXEDID_MAP_ADDR(port), 0, 256, TxBEdidRam(port)+(256*value32));
                NewValue = NewValue + 1;
                AvUapiOutputDebugMessage("Port%d: Read Segment %d",port->index, NewValue);
                GSV2K11_TXPHY_set_TX_DDC_READ_EDID_RAM_PART(port, NewValue);
                GSV2K11_TXPHY_set_TX_EDID_RE_READ(port, 0);
                GSV2K11_TXPHY_set_TX_EDID_RE_READ(port, 1);
                port->content.tx->EdidReadSuccess = AV_EDID_RESET;
            }
            ...
            (port->content.tx->EdidReadSuccess == AV_EDID_RESET))
    {
        ...
        GSV2K11_TXPHY_get_TX_EDID_TRY_TIMES(port, &value);
        if(value == 0)
        {
            AvUapiOutputDebugFsm("Port %d: Edid Fail, Retry",port->index);
            /* set flag */
            GSV2K11_TXPHY_set_TX_HDCP1P4_CLR_ERR(port, 1);
            Gsv2k11ToggleTxHpd(port);
 ...
 * @return AvOk: success
 */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiTxVideoManage(pin AvPort *port))
{
    AvRet ret = AvOk;
    uint8 value = 0;
    uint8 NewValue = 0;
    ...
    if(port->content.tx->HdmiMode == 0)
    {
        ...
        if(port->content.video->Y == AV_Y2Y1Y0_RGB)
            ...
            NewValue = 1;
        GSV2K11_TXDIG_get_TX_VIDEO_INPUT_CS(port, &value);
        if(value != NewValue)
            GSV2K11_TXDIG_set_TX_VIDEO_INPUT_CS(port, NewValue);
        /* 1.2 DVI color space is always RGB */
        GSV2K11_TXDIG_get_TX_VIDEO_OUTPUT_FORMAT(port, &value);
        if(value != 0)
            GSV2K11_TXDIG_set_TX_VIDEO_OUTPUT_FORMAT(port, 0);
        /* 1.3 color manage */
        /* 1.3.1 get CSC table */
        ...
        /* 1.3.2.1 csc enable */
        GSV2K11_TXDIG_get_TX_VIDEO_CSC_ENABLE(port, &value);
        if(value != NewValue)
            GSV2K11_TXDIG_set_TX_VIDEO_CSC_ENABLE(port, NewValue);
        /* 1.3.2.2 csc coeffs */
        if(NewValue == 1)
        {
            AvHalI2cReadField8(GSV2K11_TXDIG_MAP_ADDR(port), 0x2D, 0xFF, 0, &value);
            if(value != RegTable[1])
            {
                AvHalI2cWriteMultiField(GSV2K11_TXDIG_MAP_ADDR(port), 0x2C, 24, RegTable);
                value = (RegTable[0]>>5)&0x03;
                GSV2K11_TXDIG_set_TX_VIDEO_CSC_SCALING_FACTOR(port, value);
            }
        }
    }
    if((port->content.tx->HdmiMode == 0) || (port->content.video->info.TmdsFreq > 450))
    {
        GSV2K11_TXDIG_get_TX_GC_CD(port, &value);
        if(value != 4)
            GSV2K11_TXDIG_set_TX_GC_CD(port, 4); /* fixed to 8-bit */
    }
 ...
 * @return AvOk: success
 */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiTxAudioManage(pin AvPort *port))
{
    AvRet ret = AvOk;
    uint8 value = 0;
    uint8 TempStoreCA = 0;
    ...
    {
        GSV2K11_TXDIG_get_TX_PKT_AUDIO_SAMPLE_ENABLE(port, &value);
        if(value == 1)
            GSV2K11_TXDIG_set_TX_PKT_AUDIO_SAMPLE_ENABLE(port, 0);
    }
    else
    {
        ...
        if(port->type == HdmiTx)
            GSV2K11_INT_get_TX1_AUDIOFIFO_FULL_RAW_ST(port, &value);
        if(value == 1)
        {
            /* toggle AUDIOIF_CC */
            GSV2K11_TXPKT_get_AUD_IF_CA7_0(port, &TempStoreCA);
            value = TempStoreCA + 1;
            GSV2K11_TXPKT_set_AUD_IF_CA7_0(port, value);
            GSV2K11_TXPKT_set_AUD_IF_CA7_0(port, TempStoreCA);
            AvUapiOutputDebugMessage("Port%d: Audio Recover From Fifo Full", port->index);
            if(port->type == HdmiTx)
                GSV2K11_INT_set_TX1_AUDIOFIFO_FULL_CLEAR(port, 1);
        }
    }
 ...
 * @return AvOk: success
 */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiRxVideoManage(pin AvPort *port, VideoInterrupt* Intpt))
{
    return AvOk;
}
 ...
 * @return AvOk: success
 */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiRxAudioManage(pin AvPort *port, AudioInterrupt* Intpt))
{
    ...
    if(Intpt->InternMute)
    {
        Gsv2k11AvUapiRxSetAudioInternalMute(port);
        ret = AvOk;
    }
    if(Intpt->CsDataValid)
    {
        ...
        {
            case AV_AUD_I2S:
                GSV2K11_RXAUD_set_RX_DIS_I2S_ZERO_COMPR(port, 1);
                break;
        ...
        GSV2K11_RXAUD_set_RX_DSD_OUT_MAN_EN(port, DsdMan);
        GSV2K11_RXAUD_set_RX_DSD_OUT_MAN_SEL(port, DsdSel);
        GSV2K11_RXAUD_set_RX_HBR_OUT_MAN_EN(port, HbrMan);
        GSV2K11_RXAUD_set_RX_HBR_OUT_MAN_SEL(port, HbrSel);
        ret = AvOk;
    }
    if(Intpt->NChange)
    {
    ...
    if(Intpt->AudModeChg)
    {
        ...
        GSV2K11_RXAUD_set_DISABLE_AUDIO_MUTE(port, 1);
        GSV2K11_RXAUD_set_DISABLE_AUDIO_MUTE(port, 0);
        ret = AvOk;
    }
    ...
    */
    ...
    if(ApllRecalcFlag == 1)
    {
        GSV2K11_RXDIG_set_N_CTS_STABLE_DET_START(port, 1);
        GSV2K11_RXDIG_set_RX_APLL_MAN_START_CALC(port, 1);
    }
 ...
 * @return AvOk: success
 */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiConnectPort(pin AvPort *FromPort, pin AvPort *ToPort, AvConnectType type))
{
    AvRet ret = AvOk;
    uint8 value1 = 0;
    uint8 value2 = 0;
    ...
    if((ToPort->type == HdmiTx) && (type != AvConnectAudio))  /* Tx core with video connection */
    {
        ...
        if(ToPort->core.HdmiCore != -1)
            Gsv2k11AvUapiTxDisableCore(ToPort);
    }
    ...
    if(ToPort->index == 5)     /* TxB core */
    {
        ...
        if(type == AvConnectAudio)
        {
            ...
            Gsv2k11AvUapiTxEnableInfoFrames (ToPort, (AV_BIT_ACR_PACKET |
        ...
        else
        {
            ...
            ToPort->content.audio->AvailableAudioPackets = 0;
            Gsv2k11AvUapiTxEnableInfoFrames (ToPort, 0xffff, 0);
        }
    }
    ...
    switch(ToPort->index)
    {
        case 5:
            ...
            {
                GSV2K11_SEC_set_TXPORT_A_SRC_SEL(FromPort, 0);
                GSV2K11_SEC_set_TXPORT_B_SRC_SEL(FromPort, 0);
                switch(FromPort->index)
                {
                    case 0:
                        /* From Rx Port */
                        if(type != AvConnectAudio)
                            GSV2K11_SEC_set_TX1_SRC_SEL(FromPort, 0);
                        break;
                    case 20:
                        /* From Scaler Port */
                        GSV2K11_SEC_set_TX1_SRC_SEL(FromPort, 2);
                        break;
                    case 24:
                        /* From Color Port */
                        GSV2K11_SEC_set_TX1_SRC_SEL(FromPort, 2);
                    ...
                    case 28:
                        /* From VG */
                        GSV2K11_SEC_set_TX1_SRC_SEL(FromPort, 4);
                        break;
                    case 30:
                        /* From Video In Bus */
                        GSV2K11_SEC_set_TX1_SRC_SEL(FromPort, 3);
                }
                /* Audio Processing */
                GSV2K11_PRIM_get_TX1_AUD_SRC_SEL(FromPort, &value1);
                switch(FromPort->index)
                {
                    case 0:
                        if(type != AvConnectVideo)
                            GSV2K11_PRIM_set_TX1_AUD_SRC_SEL(FromPort, 1);
                        break;
                    case 10:
                        GSV2K11_PRIM_set_AUD1_OEN(FromPort, 0xff);
                        GSV2K11_PRIM_set_AP1_OUT_SRC_SEL(FromPort, 0);
                        GSV2K11_PRIM_set_TX1_AUD_SRC_SEL(FromPort, 5);
                        break;
                    default:
                        if(value1 != 5)
                            GSV2K11_PRIM_set_TX1_AUD_SRC_SEL(FromPort, 1);
            ...
            {
                GSV2K11_SEC_set_TXPORT_A_SRC_SEL(FromPort, 2);
                GSV2K11_SEC_set_TXPORT_B_SRC_SEL(FromPort, 2);
            }
            break;
        case 8:
            /* Mute Output */
            ...
            GSV2K11_PRIM_set_AP1_OUT_SRC_SEL(FromPort, value1);
            break;
        case 10:
            GSV2K11_PRIM_set_AUD1_OEN(FromPort, 255);
            break;
        case 20:
            /* To Scaler Port */
            switch(FromPort->index)
            {
                case 0:
                    /* From Rx Port */
                    GSV2K11_SEC_set_CP_SRC_SEL(FromPort, 0);
                    break;
                case 28:
                    /* From VG */
                    GSV2K11_SEC_set_CP_SRC_SEL(FromPort, 3);
                    break;
                case 30:
                    /* From Video In Bus */
                    GSV2K11_SEC_set_CP_SRC_SEL(FromPort, 2);
                    break;
            }
            GSV2K11_VSP_set_CP_VID_IN(FromPort, 0);
            break;
        case 24:
            /* To Color Port */
            switch(FromPort->index)
        ...
        case 32:
            ...
            switch(FromPort->index)
            {
                case 0:
                    /* From Rx Port */
                    GSV2K11_SEC_set_TX_PAR_SRC_SEL(FromPort, 0);
                    break;
                case 20:
                case 24:
                    /* From Color Port, Scaler Port */
                    GSV2K11_SEC_set_TX_PAR_SRC_SEL(FromPort, 2);
                    break;
                case 28:
                    /* From VG */
                    GSV2K11_SEC_set_TX_PAR_SRC_SEL(FromPort, 4);
                    break;
            }
            break;
    }
    ...
    if(FromPort->index == 28)
    {
        ...
        GSV2K11_PLL_set_VG_CLK_SRC_SEL(FromPort, 0);
    ...
    {
        if(FromPort->type == HdmiRx)
            GSV2K11_RXDIG_set_VPLL_MANUAL_MODE(FromPort, 0);
    }
#endif
 ...
 * @return AvOk: success
 */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiDisconnectPort(pin AvPort *port))
{
    if(port->type == HdmiTx)
    {
 ...
 * @return AvOk: success
 */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiTxSetAudioPackets(pin AvPort *port))
{
       ...
       (port->content.tx->Lock.AudioLock == 0))
    {
        ...
        Gsv2k11AvUapiTxEnableInfoFrames(port, AV_BIT_AUDIO_SAMPLE_PACKET |
    ...
    if(port->index == 5)
    {
        ...
        if(port->content.audio->AudType == AV_AUD_TYPE_SPDIF)
            ...
            value = 1;
        GSV2K11_TXDIG_set_TX_AUDIO_EXT_MCLK_ENABLE(port, value);
        /* Step 2. Select Correct Audio Input */
        if(port->content.audio->AudType == AV_AUD_TYPE_UNKNOWN)
            value = 0;
        else
            value = port->content.audio->AudType;
        GSV2K11_TXDIG_set_TX_AUDIO_MODE(port, value);
        /* Step 4. Enable Audio Interface */
        Gsv2k11_TxSetAudioInterface(port);
        /* Step 5. Enable Audio Sampling Frequency */
        Gsv2k11_TxSetAudChStatSampFreq(port);
        /* Step 6. Enable N Value */
        Gsv2k11AvUapiTxSetAudNValue(port);
        /* Finally. Enable the packet */
        Gsv2k11AvUapiTxEnableInfoFrames(port, AV_BIT_AUDIO_SAMPLE_PACKET | AV_BIT_ACR_PACKET, 1);
        ret = AvOk;
    }
 ...
 * @return AvOk: success
 */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiRxClearFlags(pio AvPort *port))
{
    AvRet ret = AvOk;
    if(port->core.HdmiCore != -1)
    {
        GSV2K11_RXAUD_set_RX_PKT_DET_RST(port, 1);
    }
    return ret;
}
 ...
 * @return AvOk: success
 */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiRxGetVideoPacketStatus(pin AvPort *port, VideoInterrupt* Intpt))
{
    uint8 Value = 0;
    uint8 MapData[43];
    ...
    AvHalI2cReadMultiField(GSV2K11_INT_MAP_ADDR(port),Value,15,MapData+0);
    if(port->core.HdmiCore == 0)
        Value = 0x50;
    AvHalI2cReadMultiField(GSV2K11_INT_MAP_ADDR(port),Value,28,MapData+15);
    ...
    else
    {
        ...
        {
            AvHalI2cReadField8(GSV2K11_RXINFO_MAP_ADDR(port),0x00,0xFF,0,&Value);
...
uapi AvRet ImplementUapi(Gsv2k11, AvUapiRxGetPacketType(pin AvPort *port))
{
    AvRet ret = AvOk;
    uint8 value = 0;
    uint8 temp = 0;

    GSV2K11_RXAUD_get_AUDIO_SAMPLE_PKT_DET(port, &value);
    if(value == 1)
    {
    ...
    {
        GSV2K11_RXAUD_get_DSD_AUDIO_PKT_DET(port, &value);
        if(value == 1)
        {
        ...
        {
            GSV2K11_RXAUD_get_DST_AUDIO_PKT_DET(port, &value);
            if(value == 1)
            {
            ...
            {
                GSV2K11_RXAUD_get_HBR_AUDIO_PKT_DET(port, &value);
                if(value == 1)
                {
                    port->content.audio->AudFormat  = AV_AUD_I2S;
    ...
    /* HBR Special Setting */
    GSV2K11_RXAUD_get_RX_SEL_I2S(port, &value);
    if(port->content.audio->AudType == AV_AUD_TYPE_HBR)
    {
    ...
    if(temp != value)
        GSV2K11_RXAUD_set_RX_SEL_I2S(port, temp);
    return ret;
}
 ...
 * @return AvOk: success
 */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiRxGetAudioPacketStatus(pin AvPort *port, AudioInterrupt* Intpt))
{
    uint8 MapData[42];
    uint8 Value = 0;
    ...
    if(port->content.rx->Lock.AudioLock)
    {
        ...
        if(port->core.HdmiCore == 0)
            ...
            Value = 0x70;
        AvHalI2cReadMultiField(GSV2K11_INT_MAP_ADDR(port),Value,27,MapData+15);
    }
    else
        return AvNotAvailable;
    ...
    if(Intpt->AudModeChg == 1)
        Gsv2k11AvUapiRxGetPacketType(port);
 ...
 * @return AvOk: success
 */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiRxGetHdcpStatus(pin AvPort *port, HdcpInterrupt* Intpt))
{
    AvRet ret = AvOk;
    uint8 value = 0;
    uint8 value1 = 0;
    ...
    /* hdcp */
    GSV2K11_INT_get_RX1_AKSV_UPDATE_INT_ST(port,       &Intpt->AksvUpdate);
    GSV2K11_INT_get_RX1_HDCP1P4_ENCRYPTED_INT_ST(port, &Intpt->Encrypted);
    GSV2K11_INT_get_RX1_AKSV_UPDATE_RAW_ST(port,       &port->content.hdcp->AksvInterrupt);
    GSV2K11_INT_get_RX1_HDCP1P4_ENCRYPTED_RAW_ST(port, &port->content.rx->VideoEncrypted);
    GSV2K11_INT_get_RX1_HDCP2P2_RX_AUTH_RAW_ST(port,   &Rx2p2Stat);
    ...
    /* Check HDCP State */
    GSV2K11_PRIM_get_RX_HDCP_CFG_MAN_EN(port, &PreHdcpManual);
    GSV2K11_PRIM_get_RX_CFG_TO_HDCP2P2(port,  &PreHdcpVersion);
            ...
            while(KfunFindVideoNextTxEnd(port, &PrevPort, &CurrentPort) == AvOk)
            {
                if(CurrentPort->type == HdmiTx)
                {
                    ...
                    else if(CurrentPort->content.hdcptx->HdcpSupport == 1)
                    {
                        ...
                        GSV2K11_TXPHY_get_TX_HDCP2P2_CAPABILITY_RB(CurrentPort, &value);
                        GSV2K11_TXPHY_get_TX_HDCP2P2_CHECKED(CurrentPort, &value1);
    ...
    if((NewHdcpManual != HdcpManual) || (NewHdcpVersion != HdcpVersion))
    {
        ...
        GSV2K11_PRIM_set_RX_HDCP_CFG_MAN_EN(port, HdcpManual);
        GSV2K11_PRIM_set_RX_CFG_TO_HDCP2P2(port,  HdcpVersion);
    }
    ...
    if(Intpt->Encrypted)
    {
        ...
        GSV2K11_INT_set_RX1_HDCP1P4_ENCRYPTED_CLEAR(port,1);
    }
    ...
    if(value != port->content.hdcp->Hdcp2p2RxRunning)
    {
        ...
        if(value == 1)
        {
            ...
            GSV2K11_RX2P2_get_RX_STREAMID_TYPE(port, &StreamType);
            if(StreamType != 0x00)
    ...
    GSV2K11_RX2P2_get_RX_DISABLE_ECC_CHECK(port, &value);
    value1 = 1 - port->content.rx->IsInputStable;
    if(value1 != value)
        GSV2K11_RX2P2_set_RX_DISABLE_ECC_CHECK(port, value1);
 ...
 * @return AvOk: success
 */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiTxGetHdcpStatus(pin AvPort *port))
{
    AvRet ret = AvOk;
    uint8 Desire1p4 = 0;
    uint8 Desire2p2 = 0;
    ...
    {
        GSV2K11_TXPHY_set_TX_HDCP1P4_ENC_EN(port, 0);
        GSV2K11_TX2P2_set_TX_HDCP2P2_ENCRYPTION_ENABLE(port, 0);
    ...
    {
        Gsv2k11SetTxHdcpVersion(port, port->content.hdcptx->HdmiStyle, 1);
    ...
    if((port->content.hdcptx->HdcpSupport == 0) &&
       (port->content.hdcptx->Hdcp2p2SinkSupport == 0))
        return AvOk;
    GSV2K11_TXPHY_get_TX_HDCP1P4_DESIRED(port, &Desire1p4);
    GSV2K11_TX2P2_get_TX_HDCP2P2_DESIRED(port, &Desire2p2);
    if((Desire1p4 != 0) || (Desire2p2 != 0))
        port->content.hdcptx->HdcpEnabled = 1;
    else
        port->content.hdcptx->HdcpEnabled = 0;
    /* 1. Check HDCP 1.4 Errors */
    GSV2K11_TXPHY_get_TX_HDCP1P4_STATE_RB(port,   &Fsm1p4State);
    GSV2K11_TXPHY_get_TX_HDCP1P4_ERROR_FLAG(port, &Error1);
    GSV2K11_TXPHY_get_TX_HDCP1P4_BKSV_READ_FLAG(port, &port->content.hdcptx->BksvReady);
...
#if AvEnableHdcp2p2Feature
    AvHalI2cReadField8(GSV2K11_TX2P2_MAP_ADDR(port), 0x0D, 0xFF, 0, &Error2);
    AvHalI2cReadField8(GSV2K11_TX2P2_MAP_ADDR(port), 0x0E, 0xFF, 0, &Error3);
    AvHalI2cReadField8(GSV2K11_TX2P2_MAP_ADDR(port), 0x14, 0xFF, 0, &Fsm2p2State);
    if(Fsm2p2State == 0x00)
    {
        AvHalI2cReadField8(GSV2K11_TX2P2_MAP_ADDR(port), 0x13, 0xFF, 0, &value);
        if(value == 0x20)
        {
            Error4 = 1;
    ...
    switch(port->content.hdcptx->HdmiStyle)
    {
        ...
        case AV_HDCP_TX_AUTO_FAIL_OUT:
            if((port->content.hdcptx->HdcpError == 2) &&
               ((Error1 != 0) || (Error4 != 0)))
            {
                Gsv2k11SetTxHdcpVersion(port, AV_HDCP_TX_1P4_FAIL_OUT, 1);
...
uapi AvRet ImplementUapi(Gsv2k11, AvUapiTxGetBksvReady(pin AvPort *port))
{
    AvRet ret = AvNotAvailable;
    uint8 value;
    GSV2K11_TXPHY_get_TX_HDCP1P4_BKSV_READ_FLAG(port, &port->content.hdcptx->BksvReady);
    /* Running_rb only set 1 after BKSV flag is cleared which makes it an indicator */
    GSV2K11_TXPHY_get_TX_HDCP1P4_RUNNING_RB(port, &value);
...
uapi AvRet ImplementUapi(Gsv2k11, AvUapiTxClearBksvReady(pin AvPort *port))
{
    AvRet ret = AvOk;
    GSV2K11_TXPHY_set_TX_HDCP1P4_CLR_BKSV_FLAG(port, 1);
    port->content.hdcptx->BksvReady = 0;
    /* The frame should only be enabled after bksv is cleared */
    /* original start */
    /* GSV2K11_TXDIG_set_TX_HDCP1P4_FRAME_ENCRYPTION(port, 1); */
    /* original end */

#if AvEnableSimplifyHdcp
    GSV2K11_TXPHY_set_TX_HDCP1P4_ENC_EN(port, 1);
#endif
...
uapi AvRet ImplementUapi(Gsv2k11, AvUapiTxClearRxidReady(pin AvPort *port))
{
    AvRet ret = AvOk;

    GSV2K11_TX2P2_set_TX_HDCP2P2_CLEAR_RXID_LIST_READY(port, 1);
    GSV2K11_TX2P2_set_TX_HDCP2P2_CLEAR_RXID_READY(port, 1);
...
uapi AvRet ImplementUapi(Gsv2k11, AvUapiTxGetSinkHdcpCapability(pio AvPort *port))
{
    AvRet ret = AvOk;
    uint8 RbState = 0;
    ...
    RbState = Gsv2k11TxScdcAction(port, 0, 0x74, 0x50, 0x00);
    if(RbState == 0x04)
    {
        port->content.hdcptx->Hdcp2p2SinkSupport = 1;
...
uapi AvRet ImplementUapi(Gsv2k11, AvUapiTxClearHdcpError(pin AvPort *port))
{
    AvRet ret = AvOk;
    if(port->content.hdcptx->Hdcp2p2SinkSupport)
    {
        GSV2K11_TX2P2_set_TX_HDCP2P2_FSM_DISABLE(port, 1);
        GSV2K11_TX2P2_set_TX_HDCP2P2_CLEAR_ERROR(port, 1);
        GSV2K11_TX2P2_set_TX_HDCP2P2_CLEAR_ERROR(port, 0);
        GSV2K11_TX2P2_set_HDCP2P2_MASTER_CLK_DIV(port, 0x14A); // lower to 30KHz DDC
        GSV2K11_TX2P2_set_TX_HDCP2P2_FSM_DISABLE(port, 0);
    }
    GSV2K11_TXPHY_set_TX_HDCP1P4_CLR_ERR(port, 1);
...
uapi AvRet ImplementUapi(Gsv2k11, AvUapiTxReadBksv(pio AvPort *port, uint8 *Value, uint8 Count))
{
    AvRet ret = AvOk;
    uint8 ReadBksv[5];
    /* only 1 sink */
    if(Count == 0)
    {
        ...
        GSV2K11_TXPHY_get_TX_HDCP1P4_BKSV0(port,&ReadBksv[0]);
        GSV2K11_TXPHY_get_TX_HDCP1P4_BKSV1(port,&ReadBksv[1]);
        GSV2K11_TXPHY_get_TX_HDCP1P4_BKSV2(port,&ReadBksv[2]);
        GSV2K11_TXPHY_get_TX_HDCP1P4_BKSV3(port,&ReadBksv[3]);
        GSV2K11_TXPHY_get_TX_HDCP1P4_BKSV4(port,&ReadBksv[4]);
        /* BKSV is inverted */
        Value[0] = ReadBksv[4];
        Value[1] = ReadBksv[3];
        Value[2] = ReadBksv[2];
    ...
    {
        AvHalI2cReadMultiField(GSV2K11_TXEDID_MAP_ADDR(port), 5*(Count-1), 5, Value);
    }
    return ret;
}

uapi AvRet ImplementUapi(Gsv2k11, AvUapiRxHdmiClearKsvFifo(pio AvPort *port))
{
    AvRet ret = AvOk;
    uint8 i,j;
    ...
    {
        GSV2K11_RXRPT_set_HDCP_MAP_SEL(port, i);
        AvHalI2cWriteMultiField(GSV2K11_RXRPT_MAP_ADDR(port), StartReg, 64, ZeroBuf);
        AvHalI2cReadMultiField(GSV2K11_RXRPT_MAP_ADDR(port), StartReg+64, 10, RdBuf);
        if (AvMemcmp(RdBuf, ZeroBuf, 10)==0)
        {
            break;
        }
        AvHalI2cWriteMultiField(GSV2K11_RXRPT_MAP_ADDR(port), StartReg+64, 64, ZeroBuf);
    }
    GSV2K11_RXRPT_set_HDCP_MAP_SEL(port, 0);
    AvUapiOutputDebugMessage("Clear Rx KSV Fifo");

    return ret;
}

uapi AvRet ImplementUapi(Gsv2k11, AvUapiRxCheckBksvExisted(pin AvPort *port, uint8 *Bksv))
{
    AvRet ret = AvNotAvailable;
    ...
    for(Position=0;Position<(port->content.hdcp->SinkTotal);Position++)
    {
        ...
        GSV2K11_RXRPT_set_HDCP_MAP_SEL(port, MapNumber);
        if(Position == 25)
        {
            AvHalI2cReadMultiField(GSV2K11_RXRPT_MAP_ADDR(port), StartReg+(uint8)(BytePtrStart&0x007F), 3, Value);
            GSV2K11_RXRPT_set_HDCP_MAP_SEL(port, MapNumber+1);
            AvHalI2cReadMultiField(GSV2K11_RXRPT_MAP_ADDR(port), StartReg+0x00, 2, Value+3);
        }
        else if(Position == 51)
        {
            AvHalI2cReadMultiField(GSV2K11_RXRPT_MAP_ADDR(port), StartReg+(uint8)(BytePtrStart&0x007F), 1, Value);
            GSV2K11_RXRPT_set_HDCP_MAP_SEL(port, MapNumber+1);
            AvHalI2cReadMultiField(GSV2K11_RXRPT_MAP_ADDR(port), StartReg+0x00, 4, Value+1);
        }
        else if(Position == 76)
        {
            AvHalI2cReadMultiField(GSV2K11_RXRPT_MAP_ADDR(port), StartReg+(uint8)(BytePtrStart&0x007F), 4, Value);
            GSV2K11_RXRPT_set_HDCP_MAP_SEL(port, MapNumber+1);
            AvHalI2cReadMultiField(GSV2K11_RXRPT_MAP_ADDR(port), StartReg+0x00, 1, Value+4);
        }
        else
            AvHalI2cReadMultiField(GSV2K11_RXRPT_MAP_ADDR(port), StartReg+(uint8)(BytePtrStart&0x007F), 5, Value);
...
uapi AvRet ImplementUapi(Gsv2k11, AvUapiRxAddBksv(pio AvPort *port, uint8 *Value, uint8 Position))
{
    AvRet ret = AvOk;
    ...
    if(Position == 25)
    {
        AvHalI2cWriteMultiField(GSV2K11_RXRPT_MAP_ADDR(port), StartReg+(uint8)(BytePtrStart&0x007F), 3, Value);
        GSV2K11_RXRPT_set_HDCP_MAP_SEL(port, MapNumber+1);
        AvHalI2cWriteMultiField(GSV2K11_RXRPT_MAP_ADDR(port), StartReg+0x00, 2, Value+3);
    }
    else if(Position == 51)
    {
        AvHalI2cWriteMultiField(GSV2K11_RXRPT_MAP_ADDR(port), StartReg+(uint8)(BytePtrStart&0x007F), 1, Value);
        GSV2K11_RXRPT_set_HDCP_MAP_SEL(port, MapNumber+1);
        AvHalI2cWriteMultiField(GSV2K11_RXRPT_MAP_ADDR(port), StartReg+0x00, 4, Value+1);
    }
    else if(Position == 76)
    {
        AvHalI2cWriteMultiField(GSV2K11_RXRPT_MAP_ADDR(port), StartReg+(uint8)(BytePtrStart&0x007F), 4, Value);
        GSV2K11_RXRPT_set_HDCP_MAP_SEL(port, MapNumber+1);
        AvHalI2cWriteMultiField(GSV2K11_RXRPT_MAP_ADDR(port), StartReg+0x00, 1, Value+4);
    }
    else
        AvHalI2cWriteMultiField(GSV2K11_RXRPT_MAP_ADDR(port), StartReg+(uint8)(BytePtrStart&0x007F), 5, Value);

    return ret;
}

uapi AvRet ImplementUapi(Gsv2k11, AvUapiTxGetBksvTotal(pio AvPort *port, uint8 *Value))
{
    AvRet ret = AvOk;
    GSV2K11_TXPHY_get_TX_HDCP1P4_BKSV_COUNT(port, Value);
 ...
 * @return AvOk: success
 */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiRxGetPacketContent(pio AvPort *port, PacketType Pkt, uint8 *Content))
{
    AvRet ret = AvOk;
    uint8 value = 0;

#if AvEnableInternalVideoGen
    if((port->type == VideoGen) || (port->type == LogicVideoRx))
#else
    ...
    switch (Pkt)
    {
        ...
        case AV_PKT_EMP_PACKET:
            AvHalI2cRdMultiField(GSV2K11_RXINFO2_MAP_ADDR(port), RxPktID[Pkt], 2, (Content+1));
            AvHalI2cRdMultiField(GSV2K11_RXINFO2_MAP_ADDR(port), RxPktPtr[Pkt], PktSize[Pkt]-3, (Content+3));
        ...
        case AV_PKT_AUDIO_CHANNEL_STATUS:
            GSV2K11_RXDIG_get_RB_RX_AUDIO_LAYOUT(port, &Content[5]);
            AvHalI2cRdMultiField(GSV2K11_RXAUD_MAP_ADDR(port), RxPktID[Pkt], 5, &Content[0]);
            GSV2K11_INT_get_RX1_CSDATA_VALID_INT_ST(port, &value);
            if (value != 0)
                GSV2K11_INT_set_RX1_CSDATA_VALID_CLEAR(port, 1);
            break;
        ...
        case AV_PKT_GMD_PACKET:
            AvHalI2cRdMultiField(GSV2K11_RXINFO_MAP_ADDR(port), RxPktID[Pkt], 3, Content);
            AvHalI2cRdMultiField(GSV2K11_RXINFO_MAP_ADDR(port), RxPktPtr[Pkt], PktSize[Pkt]-3, (Content+3));
            break;
        default:
            ret = AvNotSupport;
            break;
 ...
 * @return AvOk: success
 */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiRxClearAudioInterrupt(pio AvPort *port, AudioInterrupt* Intpt))
{
    AvRet ret = AvOk;
    ...
    if(Intpt->AudChanMode)
    {
        GSV2K11_INT_set_RX1_AUDIO_LAYOUT_CLEAR(port, 1);
    }
    /* InternMute */
    if(Intpt->InternMute)
    {
        GSV2K11_INT_set_RX1_HDMI_AVMUTE_CLEAR(port,1);
    }
    /* CsDataValid */
    if(Intpt->CsDataValid)
    {
        GSV2K11_INT_set_RX1_CSDATA_VALID_CLEAR(port,1);
        GSV2K11_INT_set_RX1_PKT_CS_CLEAR(port,1);
    }
    /* NChange */
    if(Intpt->NChange)
    {
        GSV2K11_INT_set_RX1_PKT_N_CLEAR(port,1);
    }
    /* CtsThresh */
    if(Intpt->CtsThresh)
    {
        GSV2K11_INT_set_RX1_PKTDET_ACR_CLEAR(port,1);
        GSV2K11_INT_set_RX1_PKT_CTS_CLEAR(port,1);
    }
    /* AudFifoOv */
    if(Intpt->AudFifoOv)
    {
        GSV2K11_INT_set_RX1_PKT_OVFL_CLEAR(port,1);
    }
    /* AudFifoUn */
    if(Intpt->AudFifoUn)
    {
        GSV2K11_INT_set_RX1_PKT_UNDFL_CLEAR(port,1);
    }
    /* AudFifoNrOv */
    if(Intpt->AudFifoNrOv)
    {
        GSV2K11_INT_set_RX1_PKT_ALMOST_OVFL_CLEAR(port,1);
    }
    /* AudioPktErr */
    if(Intpt->AudioPktErr)
    {
        GSV2K11_INT_set_RX1_PKT_AUDPKTERR_CLEAR(port,1);
    }
    /* AudModeChg */
    if(Intpt->AudModeChg)
    {
        GSV2K11_INT_set_RX1_CHANGE_AUDIO_MODE_CLEAR(port,1);
    }
    /* AudFifoNrUn */
    if(Intpt->AudFifoNrUn)
    {
        GSV2K11_INT_set_RX1_PKT_ALMOST_UNDFL_CLEAR(port,1);
    }
    /* AudFlatLine */
    /*
    if(Intpt->AudFlatLine)
    {
        GSV2K11_INT_set_RX1_PKT_FLATLINE_CLEAR(port,1);
    }
    */
    /* AudSampChg */
    if(Intpt->AudSampChg)
    {
        GSV2K11_INT_set_RX1_PKT_NEWFREQ_CLEAR(port,1);
    }
    /* AudPrtyErr */
    if(Intpt->AudPrtyErr)
    {
        GSV2K11_INT_set_RX1_PKT_PARITY_CLEAR(port,1);
    }
    /* AudIfValid */
    if(Intpt->AudIfValid)
    {
        GSV2K11_INT_set_RX1_PKTDET_AUD_IF_CLEAR(port,1);
        GSV2K11_INT_set_RX1_PKT_NEW_AUD_IF_CLEAR(port,1);
    }
    /* AcpValid */
    if(Intpt->AcpValid)
    {
        GSV2K11_INT_set_RX1_PKTDET_ACP_CLEAR(port,1);
        GSV2K11_INT_set_RX1_PKT_NEW_ACP_CLEAR(port,1);
    }
 ...
 * @return AvOk: success
 */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiRxClearVideoInterrupt(pio AvPort *port, VideoInterrupt* Intpt))
{
    AvRet ret = AvOk;
    ...
    if(Intpt->AvMute)
    {
        GSV2K11_INT_set_RX1_AVMUTE_CLEAR(port,1);
    }
    /* HdmiModeChg */
    if(Intpt->HdmiModeChg)
    {
        GSV2K11_INT_set_RX1_HDMI_MODE_CLEAR(port,1);
    }
    /* DeRegenLck */
    if(Intpt->DeRegenLck)
    {
    ...
    if(Intpt->Vid3dDet)
    {
        GSV2K11_INT_set_RX1_VIDEO_3D_CLEAR(port,1);
    }
    /* NewTmds */
    if(Intpt->NewTmds)
    {
    ...
    if(Intpt->BadTmdsClk)
    {
        GSV2K11_INT_set_RX1_CLKCHANGEDETECTED_CLEAR(port,1);
    }
    /* DeepClrChg */
    if(Intpt->DeepClrChg)
    {
        GSV2K11_INT_set_RX1_DEEP_COLOR_CHANGE_CLEAR(port,1);
    }
    /* PktErr */
    if(Intpt->PktErr)
    {
        GSV2K11_INT_set_RX1_PKT_PKTERR_CLEAR(port,1);
    }
    /* AvIfValid */
    if(Intpt->AvIfValid)
    {
        GSV2K11_INT_set_RX1_PKTDET_AVI_IF_CLEAR(port,1);
        GSV2K11_INT_set_RX1_PKT_NEW_AVI_IF_CLEAR(port,1);
    }
    /* SpdValid */
    if(Intpt->SpdValid)
    {
        GSV2K11_INT_set_RX1_PKTDET_SPD_IF_CLEAR(port,1);
        GSV2K11_INT_set_RX1_PKT_NEW_SPD_IF_CLEAR(port,1);
    }
    /* HdrValid */
    if(Intpt->HdrValid)
    {
        GSV2K11_INT_set_RX1_PKTDET_DRM_IF_CLEAR(port,1);
        GSV2K11_INT_set_RX1_PKT_NEW_DRM_IF_CLEAR(port,1);
    }
    /* EmpValid */
    if(Intpt->EmpValid)
    {
        GSV2K11_INT_set_RX1_PKTDET_SPR_CLEAR(port, 1);
        GSV2K11_INT_set_RX1_PKT_NEW_SPR_PKT_CLEAR(port, 1);
    }
    /* MsValid */
    if(Intpt->MsValid)
    {
        GSV2K11_INT_set_RX1_PKTDET_MPEG_IF_CLEAR(port,1);
        GSV2K11_INT_set_RX1_PKT_NEW_MPEG_IF_CLEAR(port,1);
    }
    /* VsValid */
    if(Intpt->VsValid)
    {
        GSV2K11_INT_set_RX1_PKTDET_VS_IF_CLEAR(port,1);
        GSV2K11_INT_set_RX1_PKT_NEW_VS_IF_CLEAR(port,1);
    }
    /* Isrc1Valid */
    if(Intpt->Isrc1Valid)
    {
        GSV2K11_INT_set_RX1_PKTDET_ISRC1_CLEAR(port,1);
        GSV2K11_INT_set_RX1_PKT_NEW_ISRC1_CLEAR(port,1);
    }
    /* Isrc2Valid */
    if(Intpt->Isrc2Valid)
    {
        GSV2K11_INT_set_RX1_PKTDET_ISRC2_CLEAR(port,1);
        GSV2K11_INT_set_RX1_PKT_NEW_ISRC2_CLEAR(port,1);
    }
    /* GamutValid */
    if(Intpt->GamutValid)
    {
        GSV2K11_INT_set_RX1_PKTDET_GAMUT_CLEAR(port,1);
        GSV2K11_INT_set_RX1_PKT_NEW_GAMUT_CLEAR(port,1);
    }
    /* GcValid */
    if(Intpt->GcValid)
    {
        GSV2K11_INT_set_RX1_PKTDET_GCP_CLEAR(port,1);
    }
 ...
 * @return AvOk: success
 */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiRxClearHdcpInterrupt(pio AvPort *port, HdcpInterrupt* Intpt))
{
    AvRet ret = AvOk;

    if(Intpt->AksvUpdate)
    {
        Gsv2k11IntptClr(AKSV_UPDATE);
    }
 ...
 * @return AvOk: success
 */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiTxEnableInfoFrames(AvPort* port, uint16 InfoFrames, uint8 Enable))
{
    AvRet ret = AvOk;
#if (Gsv2k11MuxMode == 0)
    if(port->core.HdmiCore == -1)
    ...
    GSV2K11_TXPKT_get_TX_PKT_UPDATE(port, &CurrentShadow);
    ...
    if (InfoFrames & AV_BIT_AV_INFO_FRAME)
    {
        ...
        if(PktShadow == 0)
            GSV2K11_TXDIG_set_TX_PKT_AVIIF_PKT_ENABLE(port, PktEn);
    }
    if (InfoFrames & AV_BIT_AUDIO_INFO_FRAME)
    {
        ...
        if(PktShadow == 0)
            GSV2K11_TXDIG_set_TX_PKT_AUDIO_INFO_ENABLE(port, PktEn);
    }
    if (InfoFrames & AV_BIT_ACP_PACKET)
    {
        ...
        if(PktShadow == 0)
            GSV2K11_TXDIG_set_TX_PKT_ACP_ENABLE(port, PktEn);
    }
    if (InfoFrames & AV_BIT_SPD_PACKET)
    {
        ...
        if(PktShadow == 0)
            GSV2K11_TXDIG_set_TX_PKT_SPDIF_ENABLE(port, PktEn);
    }
    if (InfoFrames & AV_BIT_GMD_PACKET)
    {
        ...
        if(PktShadow == 0)
            GSV2K11_TXDIG_set_TX_PKT_GM_ENABLE(port, PktEn);
    }
    if (InfoFrames & AV_BIT_MPEG_PACKET)
    {
        ...
        if(PktShadow == 0)
            GSV2K11_TXDIG_set_TX_PKT_MPEG_ENABLE(port, PktEn);
    }
    if (InfoFrames & AV_BIT_VS_PACKET)
    {
        ...
        if(PktShadow == 0)
            GSV2K11_TXDIG_set_TX_PKT_VSIF_ENABLE(port, PktEn);
    }
    if (InfoFrames & AV_BIT_ISRC1_PACKET)
    {
        ...
        if(PktShadow == 0)
            GSV2K11_TXDIG_set_TX_PKT_ISRC_ENABLE(port, PktEn);
    }
    ...
    if (InfoFrames & AV_BIT_GC_PACKET)
    {
        ...
        if(PktShadow == 0)
            GSV2K11_TXDIG_set_TX_PKT_GC_ENABLE(port, PktEn);
    }
    if (InfoFrames & AV_BIT_ACR_PACKET)
    {
        if(PktShadow == 0)
            GSV2K11_TXDIG_set_TX_PKT_N_CTS_ENABLE(port, PktEn);
    }
    if (InfoFrames & AV_BIT_AUDIO_SAMPLE_PACKET)
    {
        if(PktShadow == 0)
            GSV2K11_TXDIG_set_TX_PKT_AUDIO_SAMPLE_ENABLE(port, PktEn);
    }
    if (InfoFrames & AV_BIT_HDR_PACKET)
    {
        ...
        if(PktShadow == 0)
            GSV2K11_TXDIG_set_TX_PKT_USER0_ENABLE(port, PktEn);
    }
    if (InfoFrames & AV_BIT_EMP_PACKET)
    {
        ...
        if(PktShadow == 0)
            GSV2K11_TXDIG_set_TX_PKT_USER1_ENABLE(port, PktEn);
    }

    /* 4. Set Packet Shadow Regs */
    GSV2K11_TXPKT_set_TX_PKT_UPDATE(port, CurrentShadow);
 ...
 * @return AvOk: success
 */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiTxSetPacketContent(pio AvPort *port, PacketType Pkt, uint8 *Content, uint8 PktEn))
{
    AvRet ret = AvOk;
    ...
    if(port->index == 5)
    {
        ...
        {
            case AV_PKT_AV_INFO_FRAME:
                Gsv2k11_TxSendAVInfoFrame(port, Content, PktEn);
                break;
            case AV_PKT_AUDIO_CHANNEL_STATUS:
                GSV2K11_TXDIG_set_TX_AUDIO_CS_OVERRIDE(port, 1);
                GSV2K11_TXDIG_set_TX_AUDIO_CS_FORMAT(port,port->content.audio->Consumer);
                GSV2K11_TXDIG_set_TX_AUDIO_CS_COPYRIGHT(port,        port->content.audio->Copyright);
                GSV2K11_TXDIG_set_TX_AUDIO_CS_FORMAT_EXTRA(port,        port->content.audio->Emphasis);
                GSV2K11_TXDIG_set_TX_AUDIO_CS_MODE(port,       port->content.audio->ClkAccur);
                GSV2K11_TXDIG_set_TX_AUDIO_CS_CATEGORY_CODE(port, port->content.audio->CatCode);
                GSV2K11_TXDIG_set_TX_AUDIO_CS_SOURCE_NUMBER(port, port->content.audio->SrcNum);
                GSV2K11_TXDIG_set_TX_AUDIO_CS_WORD_LENGTH(port,   port->content.audio->WordLen);
                GSV2K11_TXDIG_set_TX_AUDIO_LAYOUT_MAN_VALUE(port, port->content.audio->Layout);
                Gsv2k11_TxSetAudChStatSampFreq(port);
                /* Audio UnMute */
                GSV2K11_TXDIG_set_TX_AUDIO_MUTE(port, 0);
                break;
            case AV_PKT_HDR_PACKET:
            case AV_PKT_EMP_PACKET:
                Gsv2k11AvUapiTxEnableInfoFrames(port, InfoFrameBit, 2);
                AvHalI2cWrMultiField(GSV2K11_TXDIG_MAP_ADDR(port), TxPktPtr[Pkt], PktSize[Pkt], Content);
                Gsv2k11AvUapiTxEnableInfoFrames(port, InfoFrameBit, PktEn);
                break;
            case AV_PKT_VS_PACKET:
            case AV_PKT_SPD_PACKET:
            case AV_PKT_AUDIO_INFO_FRAME:
                ...
                {
                    AvHalI2cWrMultiField(GSV2K11_TXCEC_MAP_ADDR(port), TxPktPtr[AV_PKT_SPARE3_PACKET], PktSize[Pkt], Content);
                    GSV2K11_TXDIG_set_TX_PKT_USER2_ENABLE(port, 1);
                    Content[0] = 0x00;
                    Content[7] = 0x13;
                }
                else
                {
                    GSV2K11_TXDIG_set_TX_PKT_USER2_ENABLE(port, 0);
                }
            case AV_PKT_MPEG_PACKET:
            case AV_PKT_ACP_PACKET:
            case AV_PKT_ISRC1_PACKET:
            ...
            case AV_PKT_GMD_PACKET:
                Gsv2k11AvUapiTxEnableInfoFrames(port, InfoFrameBit, 2);
                AvHalI2cWrMultiField(GSV2K11_TXPKT_MAP_ADDR(port), TxPktPtr[Pkt], PktSize[Pkt], Content);
                Gsv2k11AvUapiTxEnableInfoFrames(port, InfoFrameBit, PktEn);
                break;
            case AV_PKT_GC_PACKET:
                if(port->content.tx->HdmiMode == 0)
                    Gsv2k11AvUapiTxEnableInfoFrames(port, AV_BIT_GC_PACKET, 0);
                else
                    Gsv2k11AvUapiTxEnableInfoFrames(port, AV_BIT_GC_PACKET, PktEn);
                break;
            default:
                ret = AvNotSupport;
                break;
 ...
 * @return AvOk: success
 */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiRxGetHdmiAcrInfo (AvPort *port))
{
    uint8   value;

    GSV2K11_INT_get_RX1_PKTDET_ACR_RAW_ST(port, &value);

    if(value == 1)
    {
        /* Reading dummy CTS is for protection of reading N */
        /* directly reading N will result in 0 */
        /* GSV2K11_RXDIG_get_RX_AUDIO_CTS(port, &port->content.audio->CTSValue); */
        GSV2K11_RXDIG_get_RB_RX_AUDIO_N(port, &port->content.audio->NValue);
...
void Gsv2k11DisableRxHpa(pin AvPort *port)
{
    uint8 temp;
    uint8 PreviousValue;
    GSV2K11_PRIM_get_RX_RTERM_EN(port, &temp);
    PreviousValue = temp;

#if (AvAllowHpdLowEdidRead==0)
    GSV2K11_RXRPT_set_RXA_EDID_EN(port, 0);
#endif

    if(port->core.HdmiCore == 0)
    {
#if AvEnableHdcp2p2Feature
        GSV2K11_RXDIG_set_ECC_AVMUTE_MAN_EN(port, 1);
        GSV2K11_RXDIG_set_ECC_AVMUTE_MAN_EN(port, 0);
#endif
        if(port->content.hdcp->HdcpNeeded == AV_HDCP_RX_NOT_SUPPORT)
            GSV2K11_PRIM_set_RX1_HDCP_EN(port, 0);
    }
    ...
    if(temp != PreviousValue)
    {
        ...
        GSV2K11_PRIM_set_RX_HPD_MAN_VALUE(port, 0x0);         /* HPA=0 */
        GSV2K11_APLL_set_RXA_CKIO_EN_Z0(port, 0);
        GSV2K11_PRIM_set_RX_RTERM_EN(port, 0x0);              /* RTERM = 0 */
#else
        GSV2K11_PRIM_set_RX_HPD_MAN_VALUE(port, temp);        /* HPA=0 */
        GSV2K11_APLL_set_RXA_CKIO_EN_Z0(port, 0);
        GSV2K11_PRIM_set_RX_RTERM_EN(port, temp);             /* RTERM = 0 */
#endif
        GSV2K11_PRIM_set_RX_TMDSPLL_EN_OVERRIDE(port, 0x0);   /* CLK=0 */
        GSV2K11_PRIM_set_RX_CHANNEL_EN_OVERRIDE(port, 0x0);   /* CHANNEL=0 */
...
#if (AvAllowHpdLowEdidRead==0)
    GSV2K11_RXRPT_set_RXA_EDID_EN(port, 1);
#endif

    if(port->content.hdcp->HdcpNeeded != AV_HDCP_RX_NOT_SUPPORT)
    {
        if(port->core.HdmiCore == 0)
            GSV2K11_PRIM_set_RX1_HDCP_EN(port, 1);
    }
...
#if AvAllowHpdLowEdidRead
        GSV2K11_PRIM_set_RX_RTERM_EN(port, 0xf);              /* RTERM = 1 */
        GSV2K11_APLL_set_RXA_CKIO_EN_Z0(port, 1);
        GSV2K11_PRIM_set_RX_HPD_MAN_VALUE(port, 0xf);         /* HPA=1 */
#else
        GSV2K11_PRIM_set_RX_RTERM_EN(port, temp);             /* RTERM = 1 */
        GSV2K11_APLL_set_RXA_CKIO_EN_Z0(port, 1);
        GSV2K11_PRIM_set_RX_HPD_MAN_VALUE(port, temp);        /* HPA=1 */
#endif
        GSV2K11_PRIM_set_RX_TMDSPLL_EN_OVERRIDE(port, 0x3);   /* CLK=1 */
        GSV2K11_PRIM_set_RX_CHANNEL_EN_OVERRIDE(port, 0x3);   /* CHANNEL=1 */
...
{
    uint8 value = 0;
    GSV2K11_INT_get_RXA_CABLE_DETECT_RAW_ST(port, &value);
...
void Gsv2k11ToggleTmdsOut(AvPort *port, uint8 Enable)
{
    ...
    GSV2K11_TXPHY_set_TX_EN_STERM_CLOCK(port, Enable);
    GSV2K11_TXPHY_set_TX_EN_STERM_LANE0(port, Enable);
    GSV2K11_TXPHY_set_TX_EN_STERM_LANE1(port, Enable);
    GSV2K11_TXPHY_set_TX_EN_STERM_LANE2(port, Enable);

    /* TMDS CLK, Lane0, Lane1, Lane2, Ser0, Ser1, Ser2 */
    //GSV2K11_TXPHY_set_TX_TMDS_CLK_DRIVER_ENABLE(port, Enable);
    AvHalI2cWriteField8(GSV2K11_TXPHY_MAP_ADDR(port), 0xF2, 0xFF, 0x0, BulkWrite);
    if(Enable == 1)
...
void Gsv2k11SetTxHdcpVersion(pin AvPort *port, HdcpTxSupportType HdmiStyle, uint8 ForceUpdate)
{
    ...
    {
        case 5:
            GSV2K11_PRIM_get_TXB_CFG_TO_HDCP2P2(port,  &PrevHdcpValue);
            GSV2K11_PRIM_get_TXB_HDCP_CFG_MAN_EN(port, &PrevHdcpMan);
            break;
    }
    if((PrevHdcpValue != HdcpValue) || (PrevHdcpMan != HdcpMan) || (ForceUpdate == 1))
    {
        ...
        if(port->index == 5)
        {
            GSV2K11_PRIM_set_TXB_HDCP_EN(port, 0);
            GSV2K11_PRIM_set_TXB_CFG_TO_HDCP2P2(port,  HdcpValue);
            GSV2K11_PRIM_set_TXB_HDCP_CFG_MAN_EN(port, HdcpMan);
            GSV2K11_PRIM_set_TXB_HDCP_EN(port, 1);
        }
        Gsv2k11ToggleTxHpd(port);
    }
    if(HdmiStyle == AV_HDCP_TX_ILLEGAL_NO_HDCP)
        ...
        HdcpValue = 1;
    GSV2K11_TXPHY_set_TX_HDCP1P4_ENC_EN(port, HdcpValue);
    GSV2K11_TX2P2_set_TX_HDCP2P2_ENCRYPTION_ENABLE(port, HdcpValue);
}

void Gsv2k11ToggleTxHpd(pin AvPort *port)
{
    AvUapiOutputDebugFsm("Port %d: Toggle Tx HPD...", port->index);
    GSV2K11_TXPHY_set_TX_NOT_READ_EDID_EN(port, 1);
    GSV2K11_TXPHY_set_TX_HPD_SRC_SEL(port, 3); /* Always Set HPD to 1 */
    GSV2K11_TXPHY_set_HPD_FILTER_EN(port, 0);
    GSV2K11_TXPHY_set_TX_HPD_MAN_VALUE(port, 0); /* Toggle HPD to 0 */
    GSV2K11_TXPHY_set_TX_HPD_MAN_VALUE(port, 1); /* Toggle HPD to 1 */
    GSV2K11_TXPHY_set_HPD_FILTER_EN(port, 1);
    GSV2K11_TXPHY_set_TX_HPD_SRC_SEL(port, 0); /* Normal HPD Setting */
    GSV2K11_TXPHY_set_TX_EDID_TRY_TIMES(port, 0);
    GSV2K11_TXPHY_set_TX_NOT_READ_EDID_EN(port, 0);
}
...
void Gsv2k11UpdateRxCdrBandWidth(pin AvPort *port)
{
    ...
    PhyPageAddr = GSV2K11_RXLN0_MAP_ADDR(port);
...
uint8 Gsv2k11TxScdcAction(pin AvPort *port, uint8 WriteEnable, uint8 DevAddr, uint8 RegAddr, uint8 Value)
{
    ...
    GSV2K11_TXPHY_set_RX_DDC_UDP_DEV_ADDR(port, PageAddr);
    GSV2K11_TXPHY_set_RX_DDC_UDP_SUB_ADDR(port, RegAddr);
    if(WriteEnable)
        GSV2K11_TXPHY_set_RX_DDC_UDP_WRITE_DATA(port, Value);
    GSV2K11_TXPHY_set_RX_DDC_UDP_ACCESS_REQ(port, 1);
    /* 3. Wait For Complete */
    for(round=0;round<0x18;round++)
    {
        GSV2K11_TXPHY_get_RX_DDC_UDP_DONE(port, &DoneFlag);
        if(DoneFlag == 1)
            break;
    }
    ...
    {
        GSV2K11_TXPHY_get_RX_DDC_UDP_READ_DATA(port, &round);
    }
    return round;
}
...
uint8 Gsv2k11TxDDCError(pin AvPort *port)
{
    uint32 value32 = 0;
    uint8  value   = 0;
    GSV2K11_TXPHY_set_RD_CPU_PC_TRIGGER(port, 0);
    GSV2K11_TXPHY_set_RD_CPU_PC_TRIGGER(port, 1);
    GSV2K11_TXPHY_get_RB_CPU_PC_CAPTURED(port, &value32);
    GSV2K11_TXPHY_get_TX_EDID_DDC_ERROR_RB(port, &value);
    if(value == 1)
        GSV2K11_TXPHY_set_TX_HDCP1P4_CLR_ERR(port, 1);
...
void Gsv2k11ManualBksvRead(pin AvPort *port)
{
    ...
    GSV2K11_TXPHY_set_TX_HDCP1P4_DESIRED(port, 0);
    GSV2K11_TX2P2_set_TX_HDCP2P2_DESIRED(port, 0);
    /* 1. Read Sink Bksv */
    for(i=0;i<5;i++)
        RdBksv[i] = Gsv2k11TxScdcAction(port, 0, 0x74, i, 0x00);
    AvUapiOutputDebugMessage("Port %d: BKSV = %x,%x,%x,%x,%x", port->index,
                             RdBksv[0], RdBksv[1], RdBksv[2], RdBksv[3], RdBksv[4]);
    /* 2. Check HDCP 2.2 Capability */
    j = Gsv2k11TxScdcAction(port, 0, 0x74, 0x50, 0x00);
    if(j == 0x04)
        port->content.hdcptx->Hdcp2p2SinkSupport = 1;
    else
        port->content.hdcptx->Hdcp2p2SinkSupport = 0;
...
void Gsv2k11_TxSetAudioInterface (AvPort* port)
{
    ...
    /* Default 128Fs MCLK */
    GSV2K11_TXDIG_set_TX_AUDIO_MCLK_FS_RATIO(port, port->content.audio->AudMclkRatio);
    ...
    switch(port->content.audio->AudType)
    {
        ...
        case AV_AUD_TYPE_HBR:
            ...
            GSV2K11_TXDIG_set_TX_AUDIO_I2S_FORMAT(port, 0);
            GSV2K11_TXDIG_set_TX_AUDIO_PAPB_SYNC(port, 0);
            GSV2K11_TXDIG_set_TX_AUDIO_PAPB_SYNC(port, 1);
            break;
    ...
    /* I2S default */
    GSV2K11_TXDIG_set_TX_AUDIO_I2S_BLOCK_ENABLE(port, I2SBlockEnable);
    /* currently disabled, only useful when AP to Tx */
    GSV2K11_TXDIG_set_TX_AUDIO_SPDIF_ENABLE(port, SpdifBlockEnable);
    /* DSD and DST */
    GSV2K11_TXDIG_set_TDM_DESER_NUM_PINS(port, NumPins);
    GSV2K11_TXDIG_set_TDM_DESER_NUM_SLOT(port, NumSlot);
    GSV2K11_TXDIG_set_TDM_DESER_NUM_CH(port, NumCh);
    GSV2K11_TXDIG_set_DSD_EN(port, DsdBlockEnable);
    /* Encode Style */
    GSV2K11_TXDIG_set_TX_AUDIO_ENCODE_STYLE(port, EncodeStyle);
...
void Gsv2k11_TxSetAudChStatSampFreq (AvPort* port)
{
    ...
    if (AudioSfTable[i] != 0xff)
    {
        ...
        {
            GSV2K11_TXDIG_set_TX_AUDIO_I2S_MAN_SF(port, AudioSfTable[i+1]);
        }
        else
        {
            GSV2K11_TXDIG_set_TX_AUDIO_I2S_MAN_SF(port, 9);
        }
    }

}

uapi AvRet ImplementUapi(Gsv2k11, AvUapiTxSetAudNValue(AvPort* port))
{
    AvRet ret = AvOk;
    uint8 value = 0;
    if((port->type == HdmiTx) && (port->core.HdmiCore == 0))
    {
        //GSV2K11_TXDIG_set_TX_AUDIO_MAN_N(port, port->content.audio->NValue);
        value = (port->content.audio->NValue & 0x000ff) >> 0;
        AvHalI2cWriteField8(GSV2K11_TXDIG_MAP_ADDR(port), 0x05, 0xFF, 0x0, value);
        value = (port->content.audio->NValue & 0x0ff00) >> 8;
        AvHalI2cWriteField8(GSV2K11_TXDIG_MAP_ADDR(port), 0x06, 0xFF, 0x0, value);
        value = (port->content.audio->NValue & 0xff0000) >> 16;
        AvHalI2cWriteField8(GSV2K11_TXDIG_MAP_ADDR(port), 0x07, 0x0F, 0x0, value);
        /* reset audio fifo */
        GSV2K11_TXPKT_get_AUD_IF_CA7_0(port, &value);
        GSV2K11_TXPKT_set_AUD_IF_CA7_0(port, value+1);
        GSV2K11_TXPKT_set_AUD_IF_CA7_0(port, value);
    }
    return ret;
}
...
AvRet Gsv2k11_TxSendAVInfoFrame (AvPort *port, uint8 *AviPkt, uint8 PktEn)
{
    ...
    if((port->type == HdmiTx) && (port->core.HdmiCore == 0))
    {
        /* Set Video Timing in AvInfoFrame */
        FromPort = (AvPort*)(Gsv2k11FindHdmiRxFront(port));
        if(FromPort->type == LogicVideoRx)
        {
            /* Follow Parallel Input Polarity of Parallel bus -> Tx output */
            port->content.video->timing.HPolarity = 1;
            port->content.video->timing.VPolarity = 1;
        ...
        else
        {
            ...
            {
#if AvEnableInternalVideoGen
               FromPort = Gsv2k11FindHdmiRxFront(port);
        ...
        if(InY == AV_Y2Y1Y0_RGB)
            GSV2K11_TXDIG_set_TX_VIDEO_INPUT_CS(port, 0);
        else
            GSV2K11_TXDIG_set_TX_VIDEO_INPUT_CS(port, 1);
        ...
        switch (port->content.video->AspectRatio)
        {
            case 1:
                SET_AVIF_M(Packet, 1);
                GSV2K11_TXDIG_set_TX_VIDEO_ASPECT_RATIO(port, 0);
                break;
            case 2:
                SET_AVIF_M(Packet, 2);
                GSV2K11_TXDIG_set_TX_VIDEO_ASPECT_RATIO(port, 1);
                break;
            default:
                SET_AVIF_M(Packet, 0);
                GSV2K11_TXDIG_set_TX_VIDEO_ASPECT_RATIO(port, 0);
        ...
        /* Step 4. Pixel Repeat */
        Gsv2k11_TxSetManualPixelRepeat(port);
        ...
        if((FromPort->type == HdmiRx) &&
           (FromPort->content.video->Y == AV_Y2Y1Y0_YCBCR_422))
            ...
            Value = 0;
        GSV2K11_TXDIG_set_TX_VIDEO_INPUT_FORMAT(port, Value);
        switch (OutY)
        {
            case AV_Y2Y1Y0_RGB:
                SET_AVIF_Y(Packet, 0);
        ...
        GSV2K11_TXDIG_set_TX_VIDEO_OUTPUT_FORMAT(port, Value);
        /* Step 6. Set AVI Infoframe Content  */
        GSV2K11_TXPKT_set_AVI_IF_TYPE(port, 0x82);
        GSV2K11_TXPKT_set_AVI_IF_VER(port, Packet[1]);
        GSV2K11_TXPKT_set_AVI_IF_LEN(port, Packet[2]);
        Gsv2k11AvUapiTxEnableInfoFrames(port, AV_BIT_AV_INFO_FRAME, 2); /* enable AVI infoframe */
        AvHalI2cWrMultiField(GSV2K11_TXPKT_MAP_ADDR(port),
                             TxPktPtr[AV_PKT_AV_INFO_FRAME],
                             PktSize[AV_PKT_AV_INFO_FRAME]-3,
                             Packet+3);
        Gsv2k11AvUapiTxEnableInfoFrames(port, AV_BIT_AV_INFO_FRAME, PktEn); /* enable AVI infoframe */
        if(PktEn)
        {
...
AvRet Gsv2k11_TxSetManualPixelRepeat(AvPort *port)
{
    ...
    GSV2K11_TXDIG_set_TX_VIDEO_TMDS_CLK_RATIO_MAN(port, PrValue);
    GSV2K11_TXDIG_set_TX_VIDEO_PR_TO_RX_MAN(port, PrValue);
    GSV2K11_TXDIG_set_TX_VIDEO_PR_MODE(port, 2);
...
AvRet Gsv2k11_TxSetCSC (AvPort *port)
{
    AvRet ret = AvOk;
    uint8 i;
    AvVideoCs InCs  = Gsv2k11ColorCsMapping(port->content.video->InCs);
    AvVideoCs OutCs = Gsv2k11ColorCsMapping(port->content.video->OutCs);

    if ((InCs == OutCs) || (InCs == AV_CS_AUTO) || (OutCs == AV_CS_AUTO) ||
        (port->content.video->Y == AV_Y2Y1Y0_YCBCR_420))
    {
        GSV2K11_TXDIG_set_TX_VIDEO_CSC_ENABLE(port, 0);
        ret = AvOk;
    }
    else
    {
        for (i=0; CscTables[i].ConvTable; i++)
        {
            ...
            {
                AvHalI2cWrMultiField(GSV2K11_TXDIG_MAP_ADDR(port), 0x2C, 24, (uint8 *)(CscTables[i].ConvTable));
                AvHalI2cWriteField8(GSV2K11_TXDIG_MAP_ADDR(port),0x2B,0xFF,0,(CscTables[i].ConvTable[0] & 0xE0));
                ret = AvOk;
                break;
            }
        }
...
uapi AvRet ImplementUapi(Gsv2k11, AvUapiRxGetAvMute(pio AvPort *port))
{
    AvRet ret = AvOk;
    GSV2K11_RXDIG_get_RB_RX_AVMUTE_STATE(port, &(port->content.video->Mute.AvMute));
    return ret;
}

uapi AvRet ImplementUapi(Gsv2k11, AvUapiTxSetAvMute(pio AvPort *port))
{
    AvRet ret = AvOk;
#if Gsv2k11MuxMode
    GSV2K11_TXDIG_set_TX_MUX_MODE_AVMUTE_MAN(port, 0);
    GSV2K11_TXDIG_set_TX_MUX_MODE_AVMUTE_MAN_EN(port, 0);
#else
    if((port->type == HdmiTx) && (port->core.HdmiCore == 0))
    {
        if(port->content.video->Mute.AvMute == 1)
        {
            GSV2K11_TXDIG_set_TX_GC_CLEAR_AVMUTE(port, 0);
            GSV2K11_TXDIG_set_TX_GC_SET_AVMUTE(port, 1);
            AvUapiOutputDebugMessage("Port %d: AVMUTE Enable...", port->index);
        }
        else
        {
            GSV2K11_TXDIG_set_TX_GC_SET_AVMUTE(port, 0);
            GSV2K11_TXDIG_set_TX_GC_CLEAR_AVMUTE(port, 1);
...
uapi AvRet ImplementUapi(Gsv2k11, AvUapiRxGetHdmiModeSupport(pio AvPort *port))
{
    AvRet ret = AvOk;
    GSV2K11_RXDIG_get_RB_RX_HDMI_MODE(port, &(port->content.rx->HdmiMode));
    return ret;
...
uapi AvRet ImplementUapi(Gsv2k11, AvUapiTxSetHdmiModeSupport(pio AvPort *port))
{
    AvRet ret = AvOk;
    if(port->content.tx->HdmiMode == 1)
    {
        GSV2K11_TXDIG_set_TX_HDMI_MODE_OVERRIDE(port, 1);
        GSV2K11_TXDIG_set_TX_HDMI_MODE_MAN(port, 1);
    }
    else
    {
        GSV2K11_TXDIG_set_TX_HDMI_MODE_OVERRIDE(port, 1);
        GSV2K11_TXDIG_set_TX_HDMI_MODE_MAN(port, 0);
    }
    return ret;
}

uapi AvRet ImplementUapi(Gsv2k11, AvUapiTxSetFeatureSupport(pio AvPort *port))
{
    AvRet ret = AvOk;
    uint8 value = 0;
    ...
    GSV2K11_TXPHY_set_TX_SCDC_EN(port, 0);
    GSV2K11_TXPHY_set_RX_SCDC_PRESENT(port, value);
    /* Disable Tx RR, only use polling to check Scdc Flag */
    GSV2K11_TXPHY_set_TX_RR_CAPABLE(port, 1);
    GSV2K11_TXPHY_set_RX_RR_CAPABLE(port, 0);

    return ret;
}

uapi AvRet ImplementUapi(Gsv2k11, AvUapiRxGetVideoLock(pio AvPort *port))
{
    AvRet ret = AvOk;
    ...
    GSV2K11_RXDIG_get_RB_RX_TMDS_PLL_LOCKED (port, &(port->content.rx->Lock.PllLock));
    GSV2K11_RXDIG_get_RB_RX_HS_LOCKED(port, &(port->content.rx->Lock.DeRegenLock));
    GSV2K11_RXDIG_get_RB_RX_VS_LOCKED(port, &(port->content.rx->Lock.VSyncLock));
    if(port->content.rx->Lock.PllLock == 0)
        Gsv2k11RpllProtect(port);
    /* Step 2. Check whether tmds_pll_lock value has changed */
    GSV2K11_INT_get_RXA_TMDSPLL_LOCK_DET_INT_ST(port, &tmds_pll_lock_flag);
    if(tmds_pll_lock_flag)
    {
        GSV2K11_INT_set_RXA_TMDSPLL_LOCK_DET_CLEAR(port, 1);
        port->content.rx->Lock.PllLock = 0;
        port->content.rx->Lock.EqLock = 0;
        port->content.rx->Lock.AudioLock = 0;
    }
    /* Step 3. Check whether is locked */
    GSV2K11_INT_get_RX1_HS_LOCKED_INT_ST(port, &de_regen_lock_flag);
    GSV2K11_INT_get_RX1_VS_LOCKED_INT_ST(port, &v_sync_lock_flag);
    ...
    {
        GSV2K11_RXAUD_get_SPR_PKT_DET(port, &value);
        if(value == 1)
        {
            v_sync_lock_flag = 0;
            port->content.rx->Lock.VSyncLock = 1;
        ...
        {
            GSV2K11_RXAUD_get_SPD_IF_PKT_DET(port, &value);
            if(value == 1)
            {
                AvHalI2cReadMultiField(GSV2K11_RXINFO_MAP_ADDR(port), 0x2b, 3, pb_data);
    ...
    if((de_regen_lock_flag == 1) || (v_sync_lock_flag == 1))
    {
        port->content.rx->Lock.DeRegenLock = 0;
        port->content.rx->Lock.VSyncLock = 0;
        GSV2K11_INT_set_RX1_HS_LOCKED_CLEAR(port, 1);
        GSV2K11_INT_set_RX1_VS_LOCKED_CLEAR(port, 1);
    }
...
uapi AvRet ImplementUapi(Gsv2k11, AvUapiTxSetVideoTiming(pio AvPort *port))
{
    AvPort *FromPort;
    ...
    GSV2K11_TXDIG_set_MAN_POL_HS(port, 1 - port->content.video->timing.HPolarity);
    GSV2K11_TXDIG_set_MAN_POL_VS(port, 1 - port->content.video->timing.VPolarity);

    return AvOk;
}

uapi AvRet ImplementUapi(Gsv2k11, AvUapiRxGetVideoTiming(pio AvPort *port))
{
    AvRet ret = AvOk;

    GSV2K11_RXDIG_get_RB_RX_INTERLACED(port, &port->content.video->timing.Interlaced);
#if AvEnableDetailTiming
    uint32 TempValue = 0;
    GSV2K11_RXDIG_get_RB_RX_H_SYNC_WIDTH(port, &TempValue);
    port->content.video->timing.HSync = TempValue;
    GSV2K11_RXDIG_get_RB_RX_H_BACK_PORCH(port, &TempValue);
    port->content.video->timing.HBack = TempValue;
    //GSV2K11_RXDIG_get_RB_RX_H_FRONT_PORCH(port, &TempValue);
    //port->content.video->timing.HFront = TempValue;
    GSV2K11_RXDIG_get_RB_RX_V_SYNC_WIDTH_0(port, &TempValue);
    port->content.video->timing.VSync = TempValue>>1;
    GSV2K11_RXDIG_get_RB_RX_V_BACK_PORCH_0(port, &TempValue);
    if(TempValue%2 == 1)
        TempValue = TempValue + 1;
    port->content.video->timing.VBack = TempValue>>1;
    //GSV2K11_RXDIG_get_RB_RX_V_FRONT_PORCH_0(port, &TempValue);
    //port->content.video->timing.VFront = TempValue>>1;
    GSV2K11_RXDIG_get_RB_RX_H_TOTAL_WIDTH(port,&TempValue);
    port->content.video->timing.HTotal  = TempValue;
    GSV2K11_RXDIG_get_RB_RX_V_ACTIVE_HEIGHT_0(port,&TempValue);
    ...
    GSV2K11_RXDIG_get_RB_RX_V_TOTAL_0 (port,&TempValue);
    port->content.video->timing.VTotal  = TempValue>>1;
    GSV2K11_RXDIG_get_RB_RX_H_ACTIVE_WIDTH(port,&TempValue);
    ...
    GSV2K11_RXDIG_get_RB_RX_HS_POL(port, &port->content.video->timing.HPolarity);
    GSV2K11_RXDIG_get_RB_RX_VS_POL(port, &port->content.video->timing.VPolarity);
    /* 3D */
    GSV2K11_RXAUD_get_RB_RX_VIDEO_3D(port, &port->content.video->info.Detect3D);

    return ret;
}

uapi AvRet ImplementUapi(Gsv2k11, AvUapiRxGetHdmiDeepColor(pio AvPort *port))
{
    AvRet ret = AvOk;
    uint8 Value;
    if(port->type == HdmiRx)
    {
        GSV2K11_RXDIG_get_RB_RX_DEEP_COLOR_MODE(port, &Value);
        if(Value == 0)
...
uapi AvRet ImplementUapi(Gsv2k11, AvUapiTxSetHdmiDeepColor(pio AvPort *port))
{
    AvRet ret = AvOk;
    uint8 value = 0;
    AvPort *FromPort;
...
uapi AvRet ImplementUapi(Gsv2k11, AvUapiRxGetAudioInternalMute(pio AvPort *port))
{
    AvRet ret = AvOk;
    uint8 value = 0;
    GSV2K11_INT_get_RX1_HDMI_AVMUTE_RAW_ST(port, &value);
    if(value == 0)
    {
        GSV2K11_INT_get_RX1_PKT_FLATLINE_RAW_ST(port, &value);
        if(value == 1)
            GSV2K11_INT_set_RX1_PKT_FLATLINE_CLEAR(port,1);
    }
    if(port->content.audio->AudioMute != value)
    {
        GSV2K11_RXAUD_set_RX_MUTE_AUDIO(port, value);
...
uapi AvRet ImplementUapi(Gsv2k11, AvUapiRxSetAudioInternalMute(pio AvPort *port))
{
    AvRet ret = AvOk;
    return ret;
}
...
{
    uint8 RetVal;
    GSV2K11_RXDIG_get_RX_REPETITION_MAN_EN(port, &RetVal);

    if (RetVal == 1)
    {
        GSV2K11_RXDIG_get_RX_REPETITION_MAN(port, &RetVal);
    }
    else
    {
        GSV2K11_RXDIG_get_RB_RX_PIXEL_REPETITION(port, &RetVal);
    }
    return RetVal;
}

uapi AvRet ImplementUapi(Gsv2k11, AvUapiTxSetBlackMute(pio AvPort *port))
{
    AvRet ret = AvOk;
...
uapi AvRet ImplementUapi(Gsv2k11, AvUapiTxEncryptSink(pin AvPort *port))
{
    AvRet ret = AvOk;
    uint8 value;
    uint8 DviWithHdcp   = 0;
                ...
                if(port->content.hdcptx->DviStyle == AV_HDCP_TX_DVI_LOOSE)
                {
                    ...
                    if(port->content.hdcptx->Hdcp2p2SinkSupport == 1)
                    {
                        ...
                        if(port->content.hdcptx->HdmiStyle == AV_HDCP_TX_AUTO_FAIL_OUT)
                            Gsv2k11SetTxHdcpVersion(port, AV_HDCP_TX_1P4_FAIL_OUT, 0);
                        else
                            Gsv2k11SetTxHdcpVersion(port, AV_HDCP_TX_1P4_ONLY, 0);
                        GSV2K11_TXPHY_set_TX_NOT_READ_EDID_EN(port, 0);
                    }
                }
                break;
    ...
    if((port->content.tx->HdmiMode == 1) || (DviWithHdcp == 1))
    {
...
#if AvEnableSimplifyHdcp
        GSV2K11_TXPHY_get_TX_HDCP1P4_DESIRED(port, &value);
        if(value != Hdcp1p4Enable)
        {
            GSV2K11_TXPHY_set_TX_HDCP1P4_CLR_ERR(port, 1);
            GSV2K11_TXPHY_set_TX_HDCP1P4_DESIRED(port, Hdcp1p4Enable);
            GSV2K11_TXPHY_set_TX_HDCP1P4_ENC_EN(port, Hdcp1p4Enable);
            if(Hdcp1p4Enable)
                AvUapiOutputDebugMessage("Tx Port %d: Set Tx HDCP 1.4 Cipher Enable", (port->index-4));
        }
    #if AvEnableHdcp2p2Feature
        GSV2K11_TX2P2_get_TX_HDCP2P2_DESIRED(port, &value);
        if(value != Hdcp2p2Enable)
        {
            GSV2K11_TX2P2_set_TX_HDCP2P2_CLEAR_ERROR(port, 1);
            GSV2K11_TX2P2_set_TX_HDCP2P2_CLEAR_ERROR(port, 0);
            GSV2K11_TX2P2_set_TX_HDCP2P2_DESIRED(port, Hdcp2p2Enable);
            GSV2K11_TX2P2_set_TX_HDCP2P2_ENCRYPTION_ENABLE(port, Hdcp2p2Enable);
            if(Hdcp2p2Enable)
        ...
        {
            GSV2K11_TXPHY_set_TX_HDCP1P4_FRAME_ENCRYPTION(port, 1);
...
uapi AvRet ImplementUapi(Gsv2k11, AvUapiTxDecryptSink(pin AvPort *port))
{
    AvRet ret = AvOk;
    port->content.hdcptx->HdcpEnabled = 0;
    port->content.hdcptx->Authenticated = 0;
    ...
    /*
    if(port->content.hdcptx->Hdcp2p2TxRunning == 1)
        Gsv2k11TxScdcAction(port, 1, 0x74, 0x14, 0x00);
    */
#endif
    return ret;
}

uapi AvRet ImplementUapi(Gsv2k11, AvUapiRxSetHdcpEnable(pin AvPort *port))
{
    AvRet ret = AvOk;
    return ret;
}

uapi AvRet ImplementUapi(Gsv2k11, AvUapiRxSetBksvListReady(AvPort *port))
{
    AvRet ret = AvOk;
    uint8 On = 0;
    uint8 Clear = 0;
    ...
    GSV2K11_RXRPT_set_RX_KSV_LIST_NUM(port, port->content.hdcp->SinkTotal);
    GSV2K11_RXRPT_set_RX_KSV_LIST_READY(port, On);
    GSV2K11_RXRPT_set_RX_KSV_LIST_READY_CLR(port, Clear);

    return ret;
}

uapi AvRet ImplementUapi(Gsv2k11, AvUapiRxSetHdcpMode(pin AvPort *port))
{
    AvRet ret = AvOk;
    /* GSV2k11 has only 1 Hdmi Core */
    if(port->core.HdmiCore == 0)
    {
        if(port->content.hdcp->SinkTotal == 0)
        {
            GSV2K11_RXRPT_set_RX_HDCP1P4_BCAPS(port, AV_BCAPS_RECEIVER_MODE);
            GSV2K11_RXRPT_set_RX_HDCP1P4_BSTATUS(port, AV_BSTATUS_RECEIVER_MODE);
        ...
        else
        {
            ...
            GSV2K11_RXRPT_set_RX_HDCP1P4_BSTATUS(port, AV_BSTATUS_RECEIVER_MODE);
#else
            GSV2K11_RXRPT_set_RX_HDCP1P4_BCAPS(port, AV_BCAPS_REPEATER_MODE);
            GSV2K11_RXRPT_set_RX_HDCP1P4_BSTATUS(port, AV_BSTATUS_REPEATER_MODE);
#endif
...
uapi AvRet ImplementUapi(Gsv2k11, AvUapiHdcp2p2Mode(pin AvPort *port))
{
    return AvOk;
}

uapi AvRet ImplementUapi(Gsv2k11, AvUapiTxGetHdcpEnableStatus(pin AvPort *port))
{
    return AvOk;
}
 ...
 * @note
 */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiTxCecInit(pin AvPort *port))
{
#if AvEnableCecFeature /* CEC Related */
    GSV2K11_TXCEC_set_CEC_POWER_MODE(port, port->content.cec->CecEnable);
    if(port->content.cec->CecEnable == 1)
    {
        GSV2K11_TXCEC_set_CEC_TX_RETRY(port, 3);
        GSV2K11_TXCEC_set_CEC_RX_3BUFFERS_ENABLE(port, 1);
    }
#endif /* CEC Related */
    return AvOk;
...
AvRet ImplementUapi(Gsv2k11, AvUapiTxCecSetPhysicalAddr(AvPort *port))
{
    AvRet ret = AvOk;
#if AvEnableCecFeature /* CEC Related */
    uint32 phyaddr = port->content.tx->PhyAddr;
    GSV2K11_TXCEC_set_CEC_PHY_ADDR(port, phyaddr);
#endif
    return ret;
}
 ...
 * @return AvOk: success
 */
AvRet ImplementUapi(Gsv2k11, AvUapiTxCecSetLogicalAddr(AvPort *port))
{
    AvRet ret = AvOk;
#if AvEnableCecFeature /* CEC Related */
    ...
    {
        case 0:
            GSV2K11_TXCEC_set_CEC_LOGIC_ADDR0(port, LogAddr);
            DevBit = 0x01;
            break;
        case 1:
            GSV2K11_TXCEC_set_CEC_LOGIC_ADDR1(port, LogAddr);
            DevBit = 0x02;
            break;
        case 2:
            GSV2K11_TXCEC_set_CEC_LOGIC_ADDR2(port, LogAddr);
            DevBit = 0x04;
            break;
        default:
            ret = AvInvalidParameter;
    ...
    GSV2K11_TXCEC_get_CEC_LOGIC_ADDR_SELECTION(port, &Mask);
    Mask &= (~DevBit);
    if (Enable)
    {
        Mask |= DevBit;
    ...
    if (ret == AvOk)
    {
        GSV2K11_TXCEC_set_CEC_LOGIC_ADDR_SELECTION(port, Mask);
    }
#endif /* CEC Related */
    return ret;
}
...
uapi AvRet ImplementUapi(Gsv2k11, AvUapiCecRxGetStatus (AvPort* port))
{
    AvRet ret = AvNotAvailable;
#if AvEnableCecFeature /* CEC Related */
    uint8 BufferOrder = 0;
    ...
    GSV2K11_TXCEC_get_CEC_RX_BUF0_READY(port, &value0);
    GSV2K11_TXCEC_get_CEC_RX_BUF1_READY(port, &value1);
    GSV2K11_TXCEC_get_CEC_RX_BUF2_READY(port, &value2);
    if((value0 == 0) &&
       (value1 == 0) &&
       (value2 == 0))
    {
    ...
    GSV2K11_TXCEC_get_CEC_RX_BUF0_TIMESTAMP(port, &BufferOrder);
    if(BufferOrder != 0)
    {
        SelectOrder = BufferOrder;
        position = 0;
    }
    GSV2K11_TXCEC_get_CEC_RX_BUF1_TIMESTAMP(port, &BufferOrder);
    if((SelectOrder == 0) ||
       ((BufferOrder != 0) &&
    ...
    GSV2K11_TXCEC_get_CEC_RX_BUF2_TIMESTAMP(port, &BufferOrder);
    if((SelectOrder == 0) ||
       ((BufferOrder != 0) &&
       (BufferOrder < SelectOrder)))
    {
    ...
    {
        case CEC_RX_BUFFER1:
            AvHalI2cReadMultiField(GSV2K11_TXCEC_MAP_ADDR(port), 0x15, 17,
                                   port->content.cec->RxContent);
            GSV2K11_TXCEC_get_CEC_RX_BUF0_FRAME_LENGTH(port, &port->content.cec->RxLen);
            /* CEC buffer1 flag need to do toggling */
            GSV2K11_TXCEC_set_CEC_RX_CLEAR_BUF0(port,1);
            GSV2K11_TXCEC_set_CEC_RX_CLEAR_BUF0(port,0);
            ret = AvOk;
            break;
        case CEC_RX_BUFFER2:
            AvHalI2cReadMultiField(GSV2K11_TXCEC_MAP_ADDR(port), 0x27, 17,
                                   port->content.cec->RxContent);
            GSV2K11_TXCEC_get_CEC_RX_BUF1_FRAME_LENGTH(port, &port->content.cec->RxLen);
            /* CEC buffer2 flag need to do toggling */
            GSV2K11_TXCEC_set_CEC_RX_CLEAR_BUF1(port,1);
            GSV2K11_TXCEC_set_CEC_RX_CLEAR_BUF1(port,0);
            ret = AvOk;
            break;
        case CEC_RX_BUFFER3:
            AvHalI2cReadMultiField(GSV2K11_TXCEC_MAP_ADDR(port), 0x38, 17,
                                   port->content.cec->RxContent);
            GSV2K11_TXCEC_get_CEC_RX_BUF2_FRAME_LENGTH(port, &port->content.cec->RxLen);
            /* CEC buffer3 flag need to do toggling */
            GSV2K11_TXCEC_set_CEC_RX_CLEAR_BUF2(port,1);
            GSV2K11_TXCEC_set_CEC_RX_CLEAR_BUF2(port,0);
            ret = AvOk;
            break;
        default:
            break;
...
uapi AvRet ImplementUapi(Gsv2k11, AvUapiCecSendMessage(pin AvPort *port))
{
    AvRet ret = AvOk;
#if AvEnableCecFeature /* CEC Related */
    uint8 value;
    GSV2K11_TXCEC_get_CEC_TX_ENABLE(port, &value);
    if(value == 1)
        GSV2K11_TXCEC_set_CEC_TX_ENABLE(port, 0);
    ...
    else
    {
        ...
        if(port->index == 5)
        {
            GSV2K11_INT_set_TX1_TX_CEC_READY_CLEAR(port,          1);
            GSV2K11_INT_set_TX1_TX_ARBITRATION_LOST_CLEAR(port,   1);
            GSV2K11_INT_set_TX1_TX_RETRY_TIMEOUT_CLEAR(port,      1);
        }

        /* CecTxState = CEC_TX_STATE_BUSY; */
        /* 1. Action */
        AvHalI2cWriteMultiField(GSV2K11_TXCEC_MAP_ADDR(port),
                                0x00,
                                port->content.cec->TxLen,
                                port->content.cec->TxContent);
        GSV2K11_TXCEC_set_CEC_TX_FRAME_LENGTH(port, port->content.cec->TxLen);
        GSV2K11_TXCEC_set_CEC_TX_ENABLE(port, 1);
...
uapi AvRet ImplementUapi(Gsv2k11, AvUapiGetNackCount(pin AvPort *port))
{
    AvRet ret = AvOk;
#if AvEnableCecFeature /* CEC Related */
    GSV2K11_TXCEC_get_CEC_TX_NACK_COUNTER(port, &port->content.cec->NackCount);
#endif /* CEC Related */
    return ret;
}

uapi AvRet ImplementUapi(Gsv2k11, AvUapiCecTxGetStatus(pin AvPort *port))
{
    AvRet ret = AvOk;
#if AvEnableCecFeature /* CEC Related */
    if(port->index == 5)
    {
        GSV2K11_INT_get_TX1_TX_CEC_READY_INT_ST(port,        &port->content.cec->TxReady);
        GSV2K11_INT_get_TX1_TX_ARBITRATION_LOST_INT_ST(port, &port->content.cec->ArbLost);
        GSV2K11_INT_get_TX1_TX_RETRY_TIMEOUT_INT_ST(port,    &port->content.cec->Timeout);
    }

    GSV2K11_TXCEC_get_TX_LOWDRIVE_COUNTER(port,          &port->content.cec->LowDriveCount);
    GSV2K11_TXCEC_get_CEC_TX_NACK_COUNTER(port,              &port->content.cec->NackCount);
...
/* EDID functions */
/* Rx Read Edid */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiRxReadEdid(pio AvPort *port, uint8 *Value, uint16 Count))
{
    AvRet ret = AvOk;
    return ret;
}
...
/* Rx Write Edid */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiRxWriteEdid(pio AvPort *port, uint8 *Value, uint16 Count))
{
    AvRet ret = AvOk;
    ...
    /* Edid1 selection */
    GSV2K11_RXRPT_set_RX_EDID_RAM_SEL(port, 0);
    GSV2K11_RXRPT_set_RX_EDID_RAM_PAGE_SEL(port, 0);
    AvHalI2cWriteMultiField(GSV2K11_HDMI_EDID_ADDR(port), 0, 256, Value);
    if(Count > 256)
    {
        GSV2K11_RXRPT_set_RX_EDID_RAM_PAGE_SEL(port, 1);
        AvHalI2cWriteMultiField(GSV2K11_HDMI_EDID_ADDR(port), 0, 256, (Value+256));
    }
...
/* Rx Write SPA */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiRxSetSpa(pio AvPort *port, uint8 SpaLocation, uint8 *SpaValue))
{
    AvRet ret = AvOk;
    ...
    /* 1. Set SPA Location */
    /* using Edid1 */
    GSV2K11_RXRPT_set_RX_EDID1_SPA_LOC_LSB(port, SpaLocation);
    RamSel = 0;
    if(SpaLocation < 128)
    {
        GSV2K11_RXRPT_set_RX_EDID_RAM_SEL(port, RamSel);
        GSV2K11_RXRPT_set_RX_EDID_RAM_PAGE_SEL(port, 0);
        Spa = (SpaValue[0]<<8)+SpaValue[1];
        AvHalI2cWriteMultiField(GSV2K11_HDMI_EDID_ADDR(port), SpaLocation, 2, SpaValue);
        return ret;
    }
...
/* Tx Read Edid */
uapi AvRet ImplementUapi(Gsv2k11, AvUapiTxReadEdid(pio AvPort *port, uint8 *Value, uint16 Count))
{
    AvRet ret = AvOk;
    uint8 ReadSuccess = 0;
    uint16 i = 0;
    ...
    {
        GSV2K11_TXPHY_get_TX_EDID_READY_RB(port, &ReadSuccess);
        if(ReadSuccess == 0)
        {
            GSV2K11_TXPHY_get_TX_HDCP1P4_STATE_RB(port,   &FsmState);
    ...
    {
#if (AvEdidStoredInRam == 1)
        GSV2K11_TXPHY_get_TX_DDC_READ_EDID_RAM_PART(port, &value);
        if(value != 0)
        {
            ...
            AvHalI2cReadMultiField(GSV2K11_TXEDID_MAP_ADDR(port), 0, 256, TxBEdidRam(port)+(256*value));
            AvMemcpy(Value, TxBEdidRam(port), Count);
        }
        else
        {
            AvHalI2cReadMultiField(GSV2K11_TXEDID_MAP_ADDR(port), 0, Count, Value);
    ...
    if(port->content.tx->EdidReadSuccess != AV_EDID_UPDATED)
    {
        ...
        if(port->content.hdcptx->HdcpSupport == 0)
        {
            ...
            GSV2K11_TXPHY_set_EDID_ANALYZE_DONE(port, 1);
            Gsv2k11ManualBksvRead(port);
            GSV2K11_TXPHY_set_EDID_ANALYZE_DONE(port, 0);
        }
#else
...
uapi AvRet ImplementUapi(Gsv2k11, AvUapiRxEnableInternalEdid(pio AvPort *port))
{
    /* 1. enable internal Edid */
    GSV2K11_RXRPT_set_RXA_EDID_RAM_SEL(port, 0);
    GSV2K11_RXRPT_set_RX_EDID_CHECKSUM_RECALC(port, 1);
    GSV2K11_RXRPT_set_RXA_EDID_EN(port, 1);
    return AvOk;
}

uapi AvRet ImplementUapi(Gsv2k11, AvUapiTxArcEnable(pin AvPort *port, uint8 value))
{
    if(port->type == HdmiTx)
        GSV2K11_PRIM_set_TXB_ARC_PWR_DN(port, 1-value);
    return AvOk;
}

uapi AvRet ImplementUapi(Gsv2k11, AvUapiRxHdcp2p2Manage(pin AvPort *port))
{
    return AvOk;
}

uapi AvRet ImplementUapi(Gsv2k11, AvUapiTxHdcp2p2Manage(pin AvPort *port))
{
    return AvOk;
}
...
void Gsv2k11ManualCpParameter(AvPort *port)
{
    ...
    if((FromPort->type == HdmiRx) && (FromPort->core.HdmiCore != -1))
    {
        ...
        AvHalI2cReadField8(GSV2K11_RXAUD_MAP_ADDR(FromPort),0x19,0x40,0,&Y420Flag);
        if(Y420Flag == 0x40)
        {
            AvHalI2cReadField8(GSV2K11_RXINFO_MAP_ADDR(FromPort),0x01,0xE0,0,&Y420Flag);
            ...
            Y420Flag = 0;
        GSV2K11_RXDIG_get_RB_RX_H_ACTIVE_WIDTH(FromPort, &temp);
        temp = temp << Y420Flag;
        GSV2K11_VSP_get_VIN_H_ACTIVE(port, &OldParam);
        if(OldParam != temp)
            GSV2K11_VSP_set_VIN_H_ACTIVE(port, temp);
        GSV2K11_RXDIG_get_RB_RX_H_TOTAL_WIDTH(FromPort, &temp);
        temp = temp << Y420Flag;
        GSV2K11_VSP_get_VIN_H_TOTAL(port,  &OldParam);
        if(OldParam != temp)
            GSV2K11_VSP_set_VIN_H_TOTAL(port,  temp);
        GSV2K11_RXDIG_get_RB_RX_H_FRONT_PORCH(FromPort, &temp);
        temp = temp << Y420Flag;
        GSV2K11_VSP_get_VIN_H_FRONT(port,  &OldParam);
        if(OldParam != temp)
            GSV2K11_VSP_set_VIN_H_FRONT(port,  temp);
        GSV2K11_RXDIG_get_RB_RX_H_SYNC_WIDTH(FromPort, &temp);
        temp = temp << Y420Flag;
        GSV2K11_VSP_get_VIN_H_SYNC(port,   &OldParam);
        if(OldParam != temp)
            GSV2K11_VSP_set_VIN_H_SYNC(port,   temp);
        GSV2K11_RXDIG_get_RB_RX_H_BACK_PORCH(FromPort, &temp);
        temp = temp << Y420Flag;
        GSV2K11_VSP_get_VIN_H_BACK(port,   &OldParam);
        if(OldParam != temp)
            GSV2K11_VSP_set_VIN_H_BACK(port,   temp);
        GSV2K11_RXDIG_get_RB_RX_V_TOTAL_0(FromPort, &temp);
        temp = temp/2;
        GSV2K11_VSP_get_VIN_V_TOTAL(port,  &OldParam);
        if(OldParam != temp)
            GSV2K11_VSP_set_VIN_V_TOTAL(port,  temp);
        GSV2K11_RXDIG_get_RB_RX_V_ACTIVE_HEIGHT_0(FromPort, &temp);
        GSV2K11_VSP_get_VIN_V_ACTIVE(port, &OldParam);
        if(OldParam != temp)
            GSV2K11_VSP_set_VIN_V_ACTIVE(port, temp);
        GSV2K11_RXDIG_get_RB_RX_V_FRONT_PORCH_0(FromPort, &temp);
        temp = temp/2;
        GSV2K11_VSP_get_VIN_V_FRONT(port,  &OldParam);
        if(OldParam != temp)
            GSV2K11_VSP_set_VIN_V_FRONT(port,  temp);
        GSV2K11_RXDIG_get_RB_RX_V_SYNC_WIDTH_0(FromPort, &temp);
        temp = temp/2;
        GSV2K11_VSP_get_VIN_V_SYNC(port,   &OldParam);
        if(OldParam != temp)
            GSV2K11_VSP_set_VIN_V_SYNC(port,   temp);
        GSV2K11_RXDIG_get_RB_RX_V_BACK_PORCH_0(FromPort, &temp);
        if(temp%2 == 1)
            temp = temp + 1;
        temp = temp/2;
        GSV2K11_VSP_get_VIN_V_BACK(port,   &OldParam);
        if(OldParam != temp)
            GSV2K11_VSP_set_VIN_V_BACK(port,   temp);
    }
}

void Gsv2k11TxPllUnlockClear(AvPort *port)
{
    GSV2K11_PLL_set_TXA_PLL_LOCK_CLEAR(port, 1);
    GSV2K11_PLL_set_TXA_PLL_LOCK_CLEAR(port, 0);
}
...
uint8 Gsv2k11TweakCrystalFreq(AvPort *port, uint32 value32)
{
    ...
    if((value == 0x00) || (value == 0xff))
    {
        ...
        AvHalI2cReadField8(GSV2K11_PLL_MAP_ADDR(port),0x01,0xFF,0,&CrystalFreq);
        if(CrystalFreq == 0x00)
            AvHalI2cWriteField8(GSV2K11_PLL_MAP_ADDR(port),0x01,0xFF,0,0x0F);
        else
            AvHalI2cWriteField8(GSV2K11_PLL_MAP_ADDR(port),0x01,0xFF,0,0x00);
        return 1;
    }
    return 0;
}
...
void Gsv2k11MpllProtect(AvPort *port)
{
    ...
    AvHalI2cWriteField8(GSV2K11_SEC_MAP_ADDR(port),0x05,0x07,0,0x04);
    ...
    while(ValidMpllFreq == 0)
    {
        ...
        {
            AvHalI2cWriteField8(GSV2K11_SEC_MAP_ADDR(port),0xC5,0xFF,0,0xCC);
            AvHalI2cWriteField8(GSV2K11_SEC_MAP_ADDR(port),0xC6,0xFF,0,0x01);
            AvHalI2cWriteField8(GSV2K11_SEC_MAP_ADDR(port),0xC5,0xFF,0,0x0C);
        }
        else
            ValidMpllFreq = 1;
    }
...
void Gsv2k11RpllProtect(AvPort *port)
{
    ...
    {
        case 0:
            GSV2K11_PLL_get_RB_RXA_PLL_REF_PRE_DIV_FREQ(port,  &PllRb1);
            GSV2K11_PLL_get_RB_RXA_PLL_VCO_POST_DIV_FREQ(port, &PllRb2);
            break;
        case 4:
        case 5:
            GSV2K11_PLL_get_RB_TXA_PLL_REF_PRE_DIV_FREQ(port,  &PllRb1);
            GSV2K11_PLL_get_RB_TXA_PLL_VCO_POST_DIV_FREQ(port, &PllRb2);
            break;
        case 30:
        case 32:
            GSV2K11_PPLL_get_RB_PAR_PLL_REF_PRE_DIV_FREQ(port, &PllRb1);
            GSV2K11_PPLL_get_RB_PAR_PLL_VCO_POST_DIV_FREQ(port,&PllRb2);
            break;
    }
    /* 2. Compare Diff */
    if(PllRb1 > PllRb2)
    ...
    if(PllDiff>10)
    {
        ...
        {
            case 0:
                GSV2K11_APLL_set_RXA_RPLL_FASTLOCK_CTL(port, 1);
                GSV2K11_APLL_set_RXA_RPLL_FASTLOCK_CTL(port, 0);
                break;
            case 4:
            case 5:
                GSV2K11_APLL_set_TXA_RPLL_FASTLOCK_CTL(port, 1);
                GSV2K11_APLL_set_TXA_RPLL_FASTLOCK_CTL(port, 0);
                break;
            case 30:
            case 32:
                GSV2K11_PPLL_set_PAR_RPLL_FASTLOCK_CTL(port, 1);
                GSV2K11_PPLL_set_PAR_RPLL_FASTLOCK_CTL(port, 0);
                break;
        }
    }
...
void Gsv2k11ToggleDpllFreq(pin AvPort *port, uint8 index, uint8 Integer, uint8 *Fraction)
{
    ...
    AvHalI2cWriteField8(GSV2K11_AG_MAP_ADDR(port),Offset+4,0xFF,0,DefaultValue);
    AvHalI2cWriteField8(GSV2K11_AG_MAP_ADDR(port),Offset+0,0xFF,0,Fraction[0]);
    AvHalI2cWriteField8(GSV2K11_AG_MAP_ADDR(port),Offset+1,0xFF,0,Fraction[1]);
    AvHalI2cWriteField8(GSV2K11_AG_MAP_ADDR(port),Offset+2,0xFF,0,Fraction[2]);
    AvHalI2cWriteField8(GSV2K11_AG_MAP_ADDR(port),Offset+3,0xFF,0,Fraction[3]);
    AvHalI2cWriteField8(GSV2K11_AG_MAP_ADDR(port),Offset+4,0xFF,0,ToggleValue);
    AvHalI2cWriteField8(GSV2K11_AG_MAP_ADDR(port),Offset+4,0xFF,0,DefaultValue);
}
...
void Gsv2k11RxManualEQUpdate(AvPort *port, uint8 DefaultEQEnable)
{
        ...
        for(i=0;i<3;i++)
        {
            ...
            if(value == 0x10)
            {
                ...
                if(value == 1)
                {
                    ...
                    if(value == 0x81)
                    {
                        AvHalI2cReadField8(GSV2K11_PRIM_MAP_ADDR(port),0xB6,0x01,0,&value);
                        if(value != 0)
                        {
                            AvHalI2cWriteField8(GSV2K11_PRIM_MAP_ADDR(port),0xB6,0x01,0,0x00);
                            return;
                        }
                    }
            ...
            else
            {
                ...
                if(value == 0)
                {
                    AvHalI2cWriteField8(GSV2K11_PRIM_MAP_ADDR(port),0xB6,0x01,0,0x01);
                    port->content.rx->EQDelayExpire = 0;
                    EnhancedEqMode = 1;
                }
                if((port->content.rx->EQDelayExpire == 0) && (port->content.hdcp->Hdcp2p2RxRunning == 1))
                {
                    GSV2K11_RX2P2_set_RX_HDCP2P2_REAUTH_REQUEST(port, 1);
                    GSV2K11_RX2P2_set_RX_HDCP2P2_REAUTH_REQUEST(port, 0);
                }
            }
    ...
    if(EqDoneFlag == 3)
    {
        ...
        if(port->content.rx->EQDelayExpire == 1)
        {
            GSV2K11_RXDIG_set_DCFIFO_RECENTER(port, 1);
            GSV2K11_RXAUD_set_RX_AUD_FIFO_RST(port, 1);
            GSV2K11_RXAUD_set_RX_AUD_FIFO_RST(port, 0);
        }
    ...
    if(port->content.rx->EQDelayExpire == 0)
    {
           ...
           (EnhancedEqMode == 1) && (EnhancedFirstRound == 0))
        {
               ...
               (RbEQParam[3] == EQParam[3]))
            {
                ...
                GSV2K11_RXDIG_get_RX_SCDC_TMDS_BIT_CLK_RATIO_MAN_EN(port, pval);
                GSV2K11_RXDIG_set_RX_SCDC_TMDS_BIT_CLK_RATIO_MAN_EN(port, val);
                GSV2K11_RXDIG_get_RX_SCDC_TMDS_BIT_CLK_RATIO_MAN(port, pval);
                GSV2K11_RXDIG_set_RX_SCDC_TMDS_BIT_CLK_RATIO_MAN(port, val);
                GSV2K11_RXDIG_get_RX_SCDC_SCR_EN_MAN_EN(port, pval);
                GSV2K11_RXDIG_set_RX_SCDC_SCR_EN_MAN_EN(port, val);
                GSV2K11_RXDIG_get_RX_SCDC_SCR_EN_MAN(port, pval);
                GSV2K11_RXDIG_set_RX_SCDC_SCR_EN_MAN(port, val);
                 */
                AvHalI2cReadField8(GSV2K11_RXDIG_MAP_ADDR(port),0xD9,0xFF,0,&value);
                if(value == 0xF0)
                    value = 0xA0;
                else
                    value = 0xF0;
                AvHalI2cWriteField8(GSV2K11_RXDIG_MAP_ADDR(port),0xD9,0xFF,0,value);
        ...
        /* Automatic EQ Register Setting Recovery */
        if((port->core.HdmiCore != -1) &&
           ((EnhancedEqMode == 0) || (EnhancedFirstRound == 1)))
        {
            AvHalI2cWriteField8(GSV2K11_RXDIG_MAP_ADDR(port),0xD9,0xFF,0,0x00);
        }
        Gsv2k11UpdateRxCdrBandWidth(port);
        ...
        {
            case 0:
                GSV2K11_SEC_set_RXA_FIFO_EN(port, 0);
                GSV2K11_SEC_set_RXA_FIFO_EN(port, 1);
                break;
            case 1:
                GSV2K11_SEC_set_RXB_FIFO_EN(port, 0);
                GSV2K11_SEC_set_RXB_FIFO_EN(port, 1);
                break;
        }
        */
...
uapi AvRet ImplementUapi(Gsv2k11, AvUapiCheckLogicAudioRx(pin AvPort *port))
{
    ...
    while(KfunFindAudioNextTxEnd(port, &PrevPort, &CurrentPort) == AvOk)
    {
        if(CurrentPort->type == HdmiTx)
        {
            ...
            {
                GSV2K11_TXDIG_get_TX_AUDIO_SPDIF_SF_RB(CurrentPort, &value);
                i = 0;
            ...
            if(CurrentPort->core.HdmiCore == 0)
                GSV2K11_INT_set_TX1_AUDIOFIFO_FULL_CLEAR(CurrentPort, 1);
        }
        PrevPort = CurrentPort;
    }
#endif
...
uapi AvRet ImplementUapi(Gsv2k11, AvUapiCheckLogicAudioTx(pin AvPort *port))
{
    AvPort *TempPort;
    uint8 i = 0;
    uint8 OldValue = 0;
    ...
    {
        GSV2K11_PRIM_get_AUD1_OEN(port, &OldValue);
        if(NewValue != OldValue)
        {
            GSV2K11_PRIM_set_AUD1_OEN(port, NewValue);
            if((NewValue == 0) && (port->content.audio->WordLen != 0))
            {
                GSV2K11_RXAUD_set_WORD_LEN_MAN_EN(TempPort, 1);
                GSV2K11_RXAUD_set_WORD_LEN_MAN(TempPort, port->content.audio->WordLen);
            }
        }
    }
    ...
    if((NewValue == 0) && (TempPort->type == HdmiRx))
    {
        ...
        GSV2K11_RXDIG_get_RX_AUDIO_MCLK_X_N(TempPort, &OldValue);
        if(NewValue != OldValue)
            GSV2K11_RXDIG_set_RX_AUDIO_MCLK_X_N(TempPort, NewValue);
    }
...
void Gsv2k11CpCscManage(pin AvPort *port, AvVideoCs VarInCs, AvVideoCs VarOutCs)
{
    ...
    InCs  = Gsv2k11ColorCsMapping(VarInCs);
    OutCs = Gsv2k11ColorCsMapping(VarOutCs);
    GSV2K11_SEC_get_CP_CSC_BYPASS(port, &value1);
    if((VarInCs == AV_CS_LIM_BT2020_YCC) || (VarInCs == AV_CS_LIM_BT2020_RGB) ||
       (VarInCs == AV_CS_BT2020_YCC) || (VarInCs == AV_CS_BT2020_RGB))
        NewValue1 = 1;
    else if(InCs != OutCs)
        NewValue1 = 0;
    else
        NewValue1 = 1;
    if(value1 != NewValue1)
        GSV2K11_SEC_set_CP_CSC_BYPASS(port, NewValue1);
    if(NewValue1 == 0)
    {
        /* 3. Set CSC Value */
        ...
        {
            if(value1 == 0)
                GSV2K11_SEC_set_CP_CSC_BYPASS(port, 1);
        }
        else
        {
            /* 3.2 Write Table */
            AvHalI2cReadMultiField(GSV2K11_VSP_MAP_ADDR(port), 0x20, 24, CurrentCscTable);
            NewValue1 = 0;
            ...
            if(NewValue1 == 1)
            {
                AvHalI2cWriteMultiField(GSV2K11_VSP_MAP_ADDR(port), 0x20, 24, RegTable);
                value1 = (RegTable[0]>>5)&0x03;
                GSV2K11_VSP_set_CP_CSC_MODE(port, value1);
            }
        }
    }
}
...
uapi AvRet ImplementUapi(Gsv2k11, AvUapiCheckVideoScaler(pin AvPort *port))
{
    AvPort *FromPort = (AvPort*)port->content.RouteVideoFromPort;
    AvPort *CurrentPort = NULL;
    AvPort *PrevPort = NULL;
    AvVideoCs ScalerInCs = AV_CS_AUTO;
    AvVideoCs ScalerOutCs = AV_CS_AUTO;
    ...
    {
        GSV2K11_SEC_get_CP_MODE(port,&value1);
        if((value1 == 2) || (value1 == 3) || (value1 == 4))
            GSV2K11_SEC_set_CP_MODE(port,5);
        return AvOk;
    }
    ...
    /* 3. Check Vid In */
    GSV2K11_VSP_get_CP_VID_IN(port, &value1);
    if(value1 != InVic)
    {
        port->content.scaler->ScalerOutVic = OutVic;
        GSV2K11_VSP_set_CP_VID_IN(port, InVic);
    }
    /* 3.1 Check Manual Setting */
    GSV2K11_VSP_get_SCAL_VIN_PARAM_MAN_EN(port, &value1);
    ...
    if(value1 != value2)
    {
        GSV2K11_VSP_set_SCAL_VIN_PARAM_MAN_EN(port, value2);
        GSV2K11_VSP_set_SCAL_VOUT_PARAM_SEL(port, value2);
    }
    ...
    if(value1 != NewValue1)
    {
        GSV2K11_SEC_set_CP_MODE(port, NewValue1);
    }
    /* 5. Set Function Mode */
    GSV2K11_VSP_get_CP_VOUT_CHN_SWAP(port, &value1);
    GSV2K11_VSP_get_CP_VIN_CHN_SWAP(port, &value2);
    switch(port->content.scaler->ColorSpace)
    {
        case AV_Y2Y1Y0_YCBCR_420:
    ...
    {
        GSV2K11_VSP_set_CP_VOUT_CHN_SWAP(port, NewValue1);
        GSV2K11_VSP_set_CP_VIN_CHN_SWAP(port,  NewValue2);
    }
    /* 6. Check CP CSC State */
    /* 6.1 Check Downstream has Tx Par Port with 422 setting (embedded sync) */
    while(KfunFindVideoNextTxEnd(port, &PrevPort, &CurrentPort) == AvOk)
    {
    ...
    if((LogicVideoTxFlag == 1) &&
       (FromPort->type == HdmiRx) && (FromPort->content.rx->IsInputStable == 1))
    {
        ScalerInCs  = FromPort->content.video->InCs;
        if((FromPort->content.video->InCs == AV_CS_LIM_BT2020_YCC) ||
           (FromPort->content.video->InCs == AV_CS_LIM_BT2020_RGB))
        {
            ScalerInCs  = AV_CS_LIM_YUV_601;
        }
        if((port->content.scaler->ColorSpace == AV_Y2Y1Y0_YCBCR_420) ||
           (CurrentPort->content.video->Y == AV_Y2Y1Y0_INVALID))
        {
            ScalerOutCs = ScalerInCs;
        }
        else if(CurrentPort->content.video->InCs == AV_CS_AUTO)
        {
            if(CurrentPort->content.video->Y == AV_Y2Y1Y0_RGB)
                ScalerOutCs = AV_CS_LIM_RGB;
            else
                ScalerOutCs = AV_CS_LIM_YUV_709;
        }
        else
            ScalerOutCs = CurrentPort->content.video->InCs;
        /* Configure CP CSC */
        Gsv2k11CpCscManage(port, ScalerInCs, ScalerOutCs);
    }

    return AvOk;
}

uapi AvRet ImplementUapi(Gsv2k11, AvUapiCheckVideoColor(pin AvPort *port))
{
    uint8 value1 = 0;
    uint8 value2 = 0;
    uint8 NewValue1 = 0;
    ...
    if(value1 != port->content.color->ColorInVic)
        GSV2K11_VSP_set_CP_VID_IN(port, port->content.color->ColorInVic);
    /* 2. Check Input Color Space, Convert to YUV anyway */
    Gsv2k11CpCscManage(port, port->content.color->ColorInCs, port->content.color->ColorOutCs);
    /* 4. Check Function Enable State */
    GSV2K11_SEC_get_CP_MODE(port,&value1);
    ...
    if(((port->content.color->ColorOutSpace == AV_Y2Y1Y0_YCBCR_420) &&
       ...
       ((port->content.color->ColorOutSpace != AV_Y2Y1Y0_YCBCR_420) &&
        ...
        NewValue1 = 0;
    temp = 0;
    while(Gsv2k11VideoColorAutoVicTable[temp] != 0xff)
    {
        /* 6.1 Found Auto Conversion Vic */
        if(port->content.color->ColorInVic == Gsv2k11VideoColorAutoVicTable[temp])
        {
            NewValue1 = 0;
            break;
        }
    ...
    GSV2K11_VSP_get_CP_VIN_PARAM_MAN_EN(port, &value1);
    if(value1 != NewValue1)
        GSV2K11_VSP_set_CP_VIN_PARAM_MAN_EN(port, NewValue1);
...
uapi AvRet ImplementUapi(Gsv2k11, AvUapiCheckVideoGen(pin AvPort *port))
{
#if AvEnableInternalVideoGen
    AvPort *TempPort = NULL;
    uint8  i = 0;
    ...
    if(port->core.HdmiCore != -1)
    {
        GSV2K11_RXDIG_get_VPLL_MANUAL_MODE(port, &Value);
        if(Value != NewValue)
            GSV2K11_RXDIG_set_VPLL_MANUAL_MODE(port, NewValue);
    }
    GSV2K11_VSP_get_DG_EN(port, &Value);
    if(Value != NewValue)
    {
        GSV2K11_VSP_set_DG_EN(port, NewValue);
        if(NewValue == 0)
            GSV2K11_VSP_set_DG_VID(port, 0);
    }
    ...
    if(NewValue == 1)
    {
        ...
        while(VideoGenVicTable[i] != 0xff)
        {
            if(VideoGenVicTable[i] == port->content.video->timing.Vic)
            {
                ...
                GSV2K11_VSP_get_DG_VID(port, &Value);
                if(Value != NewValue)
                {
                    GSV2K11_VSP_set_DG_VID(port, NewValue);
                    if(port->core.HdmiCore == 0)
                    {
                        NewValue = VideoGenVicTable[i+2];
                        /* manual Channel-0 clock setting */
                        AvHalI2cWriteField8(GSV2K11_AG_MAP_ADDR(port),0x70,0xF0,0,0x00);
                        Gsv2k11ToggleDpllFreq(port, 0, NewValue, (uint8*)(VideoGenVicTable+i+3));
                    }
                }
        ...
        /* 3. Check Pattern */
        GSV2K11_VSP_get_DG_PATTERN(port, &Value);
        if(Value != port->content.vg->Pattern)
            GSV2K11_VSP_set_DG_PATTERN(port, port->content.vg->Pattern);
        /* 4. Check 420 */
        GSV2K11_VSP_get_DG_420_EN(port, &Value);
        NewValue = 0;
        if(Value != NewValue)
            GSV2K11_VSP_set_DG_420_EN(port, NewValue);
    }
...
uapi AvRet ImplementUapi(Gsv2k11, AvUapiCheckAudioGen(pin AvPort *port))
{
    return AvOk;
}

uapi AvRet ImplementUapi(Gsv2k11, AvUapiCheckClockGen(pin AvPort *port))
{
        ...
        if(port->content.cg->Update[i] == 1)
        {
            switch(i)
            {
                case 0:
                    ...
                    AvHalI2cWriteField8(GSV2K11_AG_MAP_ADDR(port),0x70,0xF0,0,ClockGenMode);
                    GSV2K11_TXPHY_set_TX_TMDS_CLK_DRIVER_ENABLE(port, 1);
                    break;
                case 1:
                    ClockGenMode = (port->content.cg->type[i]) << 1;
                    AvHalI2cWriteField8(GSV2K11_AG_MAP_ADDR(port),0x70,0x0F,0,ClockGenMode);
                    GSV2K11_TXPHY_set_TX_TMDS0_DRIVER_ENABLE(port, 1);
                    break;
                case 2:
                    ClockGenMode = (port->content.cg->type[i]) << 5;
                    AvHalI2cWriteField8(GSV2K11_AG_MAP_ADDR(port),0x71,0xF0,0,ClockGenMode);
                    GSV2K11_TXPHY_set_TX_TMDS1_DRIVER_ENABLE(port, 1);
                    break;
                case 3:
                    ClockGenMode = (port->content.cg->type[i]) << 1;
                    AvHalI2cWriteField8(GSV2K11_AG_MAP_ADDR(port),0x71,0x0F,0,ClockGenMode);
                    GSV2K11_TXPHY_set_TX_TMDS2_DRIVER_ENABLE(port, 1);
                    break;
            }
            ...
            Gsv2k11ToggleDpllFreq(port, i, port->content.cg->Integer[i], Fraction);
...
uapi AvRet ImplementUapi(Gsv2k11, AvUapiCheckLogicVideoTx(pin AvPort *port))
{
    ...
    if(port->content.lvtx->Update == 1)
    {
        if(port->content.lvtx->Config == 0)
        {
            GSV2K11_PAR_set_TX_PAR_EN(port, 0);
        }
        else if(port->content.lvtx->Config <= 64)
        {
            /* Restore Default */
            GSV2K11_PAR_set_RX_PAR_EN(port, 0);
            GSV2K11_PAR_set_TX_PAR_EN(port, 0);
            for(k=0; Gsv2k11ParTxTable[k]!=0xFF; k=k+2)
                AvHalI2cWriteField8(GSV2K11_PAR_MAP_ADDR(port),Gsv2k11ParTxTable[k],0xFF,0,Gsv2k11ParTxTable[k+1]);
            /* Configuration */
            Offset = port->content.lvtx->Config * 3;
            /* Tx Bus Configuration Bits */
            AvHalI2cWriteField8(GSV2K11_PAR_MAP_ADDR(port),0x60,0xFF,0,ParallelConfigTable[Offset+0]);
            AvHalI2cWriteField8(GSV2K11_PAR_MAP_ADDR(port),0x61,0xFF,0,ParallelConfigTable[Offset+1]);
            AvHalI2cWriteField8(GSV2K11_PAR_MAP_ADDR(port),0x62,0xFF,0,ParallelConfigTable[Offset+2]);
            /* Extra 12-bit TTL drive enable to save power */
            if(((ParallelConfigTable[Offset+0] & 0x30) != 0) &&
               ((ParallelConfigTable[Offset+2] & 0x80) == 0))
                GSV2K11_PAR_set_PAR_PHY_EN_TTL_H12B(port, 1);
            else
                GSV2K11_PAR_set_PAR_PHY_EN_TTL_H12B(port, 0);
            ...
            {
                /* HIS mode Bit flip */
                if((ParallelConfigTable[Offset+2] & 0x40) != 0)
                {
                    AvHalI2cWriteField8(GSV2K11_PAR_MAP_ADDR(port),0x66,0x01,0,0x01);
                }
                else
                {
                    AvHalI2cWriteField8(GSV2K11_PAR_MAP_ADDR(port),0x66,0x01,0,0x00);
                }
                /* TTL 2x Clock configuration */
                if(((ParallelConfigTable[Offset+0] & 0x07) == 0x02) && ((ParallelConfigTable[Offset+2] & 0x80) == 0x00))
                    AvHalI2cWriteField8(GSV2K11_PAR_MAP_ADDR(port),0x66,0x04,0,0x04);
                else
                    AvHalI2cWriteField8(GSV2K11_PAR_MAP_ADDR(port),0x66,0x04,0,0x00);
                /* To Video Out Bus */
                switch(FromPort->index)
                {
                    case 0:
            ...
            GSV2K11_SEC_set_TX_PAR_SRC_SEL(port, Value);
            /* Enable Parallel TxPort */
            GSV2K11_PAR_set_TX_PAR_EN(port, 1);
        }
        port->content.lvtx->Update = 0;
    }
    /* 2. Check Pll Status */
    FromPort = (AvPort*)(Gsv2k11FindHdmiRxFront(port));
    if((FromPort->type == HdmiRx) || (FromPort->type == VideoGen))
    {
        if(FromPort->content.rx->IsFreeRun == 1)
        {
            GSV2K11_PPLL_get_RB_PAR_PLL_LOCK(port, &Value);
            if(Value == 1)
            {
                if(port->content.lvtx->Lock == 0)
                {
                    AvUapiOutputDebugMessage("Port %d: Reset Tx PAR Interface", port->index);
                    /* Reset Style 1: Parallel Pll reset + Phy reset, grand reset */
                    /*
                    GSV2K11_PAR_set_TX_PAR_EN(port, 0);
                    GSV2K11_PAR_set_TX_PAR_EN(port, 1);
                    */
                    /* Reset Style 2: Parallel Phy reset, minor reset */
                    AvHalI2cWriteField8(GSV2K11_PAR_MAP_ADDR(port),0x02,0xFF,0,0x80);
                    AvHalI2cWriteField8(GSV2K11_PAR_MAP_ADDR(port),0x02,0xFF,0,0xC0);
                    AvHalI2cWriteField8(GSV2K11_PAR_MAP_ADDR(port),0x02,0xFF,0,0xE0);
                    AvHalI2cWriteField8(GSV2K11_PAR_MAP_ADDR(port),0x02,0xFF,0,0x00);
                }
                port->content.lvtx->Lock = 1;
            }
            else
            {
                Gsv2k11RpllProtect(port);
                port->content.lvtx->Lock = 0;
            }
        }
        else
            port->content.lvtx->Lock = 0;
...
uapi AvRet ImplementUapi(Gsv2k11, AvUapiCheckLogicVideoRx(pin AvPort *port))
{
#if AvEnableVideoLogicBus
    uint8 Offset = 0;
    uint8 value = 0;
    ...
    uint32 k = 0;
    /* 1. Configuration Update */
    if(port->content.lvrx->Update == 1)
    {
        if(port->content.lvrx->Config == 0)
        {
            GSV2K11_PAR_set_RX_PAR_EN(port, 0);
        }
        else if(port->content.lvrx->Config <= 64)
        {
            /* Restore Default */
            GSV2K11_PAR_set_TX_PAR_EN(port, 0);
            GSV2K11_PAR_set_RX_PAR_EN(port, 0);
            for(k=0; Gsv2k11ParRxTable[k]!=0xFF; k=k+2)
                AvHalI2cWriteField8(GSV2K11_PAR_MAP_ADDR(port),Gsv2k11ParRxTable[k],0xFF,0,Gsv2k11ParRxTable[k+1]);
            /* Configuration */
            Offset = port->content.lvrx->Config * 3;
            /* Rx Bus Configuration Bits */
            AvHalI2cWriteField8(GSV2K11_PAR_MAP_ADDR(port),0x20,0xFF,0,ParallelConfigTable[Offset+0]);
            AvHalI2cWriteField8(GSV2K11_PAR_MAP_ADDR(port),0x21,0xFF,0,ParallelConfigTable[Offset+1]);
            AvHalI2cWriteField8(GSV2K11_PAR_MAP_ADDR(port),0x22,0xFF,0,ParallelConfigTable[Offset+2]);
            /* UDP Configuration in Par Bus -> Tx Port */
            if((ParallelConfigTable[Offset+2] & 0x20) != 0)
            {
                GSV2K11_SEC_set_TXPORT_B_SRC_SEL(port, 3);
                AvHalI2cWriteField8(GSV2K11_TXPHY_MAP_ADDR(port),0xDF,0xFF,0,0x42);
            }
            else
            {
                GSV2K11_SEC_set_TXPORT_B_SRC_SEL(port, 0);
                AvHalI2cWriteField8(GSV2K11_TXPHY_MAP_ADDR(port),0xDF,0xFF,0,0x02);
                /* HIS mode Bit flip */
                if((ParallelConfigTable[Offset+2] & 0x40) != 0)
                {
                    AvHalI2cWriteField8(GSV2K11_PAR_MAP_ADDR(port),0x26,0xFF,0,0x01);
                }
                else
                {
                    AvHalI2cWriteField8(GSV2K11_PAR_MAP_ADDR(port),0x26,0xFF,0,0x00);
                }
                /* TTL 2x Clock configuration */
                if(((ParallelConfigTable[Offset+0] & 0x07) == 0x02) && ((ParallelConfigTable[Offset+2] & 0x80) == 0x00))
                    AvHalI2cWriteField8(GSV2K11_PAR_MAP_ADDR(port),0x26,0x04,0,0x04);
                else
                    AvHalI2cWriteField8(GSV2K11_PAR_MAP_ADDR(port),0x26,0x04,0,0x00);
            }
            /* Rx Set Vic */
            if((ParallelConfigTable[Offset+2] & 0x50) != 0)
            {
                if(port->content.video->timing.HActive == 0)
                {
                    GSV2K11_PAR_set_RX_ITU_DECODE_RESOLUTION(port, port->content.video->timing.Vic);
                }
                else
                {
                    /* Manual ITU setting */
                    GSV2K11_PAR_set_RX_ITU_DECODE_RESOLUTION(port, 0);
                    /* Horizontal Timing Information */
                    GSV2K11_PAR_set_RX_ITU_DECODE_H_POL(port, port->content.video->timing.HPolarity);
                    GSV2K11_PAR_set_RX_ITU_DECODE_H_TOTAL(port, port->content.video->timing.HTotal);
                    GSV2K11_PAR_set_RX_ITU_DECODE_H_BACK(port, port->content.video->timing.HBack);
                    value32 = port->content.video->timing.HTotal
                            - port->content.video->timing.HActive;
                    GSV2K11_PAR_set_RX_ITU_DECODE_H_BLANK(port, value32);
                    value32 = port->content.video->timing.HTotal
                            - port->content.video->timing.HActive
                            - port->content.video->timing.HSync
                            - port->content.video->timing.HBack;
                    GSV2K11_PAR_set_RX_ITU_DECODE_H_FRONT(port, value32);
                    /* Vertical Timing Information */
                    GSV2K11_PAR_set_RX_ITU_DECODE_V_POL(port, port->content.video->timing.VPolarity);
                    GSV2K11_PAR_set_RX_ITU_DECODE_V_TOTAL1(port, port->content.video->timing.VTotal);
                    GSV2K11_PAR_set_RX_ITU_DECODE_V_TOTAL2(port, port->content.video->timing.VTotal);
                    GSV2K11_PAR_set_RX_ITU_DECODE_V_SYNC1(port, port->content.video->timing.VSync);
                    GSV2K11_PAR_set_RX_ITU_DECODE_V_SYNC2(port, port->content.video->timing.VSync);
                    value32 = port->content.video->timing.VTotal
                            - port->content.video->timing.VActive;
                    GSV2K11_PAR_set_RX_ITU_DECODE_V_BLANK1(port, value32);
                    GSV2K11_PAR_set_RX_ITU_DECODE_V_BLANK2(port, value32);
                    value32 = port->content.video->timing.VTotal
                            - port->content.video->timing.VActive
                            - port->content.video->timing.VSync
                            - port->content.video->timing.VBack;
                    GSV2K11_PAR_set_RX_ITU_DECODE_V_FRONT1(port, value32);
                    GSV2K11_PAR_set_RX_ITU_DECODE_V_FRONT2(port, value32);
                }
            }
            /* Enable Parallel TxPort */
            GSV2K11_PAR_set_RX_PAR_EN(port, 1);
        }
        port->content.lvrx->Update = 0;
    }
    /* 2. Locking Update */
    /* 2.1 Clock Lock Detection */
    LockStable = 0;
    GSV2K11_PPLL_get_RB_PAR_PLL_LOCK(port, &value);
    if(value == 1)
    {
        GSV2K11_PPLL_get_RB_PAR_PLL_LOCK_CAPTURED(port, &value);
        if(value == 1)
            LockStable = 1;
        else
        {
            GSV2K11_PPLL_set_PAR_PLL_LOCK_CLEAR(port, 1);
            GSV2K11_PPLL_set_PAR_PLL_LOCK_CLEAR(port, 0);
        }
    }
    if(LockStable == 0)
    {
    ...
    {
         if(port->content.lvrx->Lock == 0)
         {
             GSV2K11_PRIM_set_RX_PAR_RST(port, 1);
             GSV2K11_PRIM_set_RX_PAR_RST(port, 0);
             GSV2K11_PPLL_get_RB_PAR_PLL_REF_CLK_FREQ(port, &value32);
             value32 = value32>>8;
             port->content.video->info.TmdsFreq = value32;
             AvUapiOutputDebugFsm("Port %d: Clock Pin Frequency = %d",port->index, value32);
         }
    }
    if((port->content.lvrx->Lock == 1) && (LockStable == 0))
        AvUapiOutputDebugFsm("Port %d: PAR PLL Unlock",port->index);
    port->content.lvrx->Lock           = LockStable;
    port->content.rx->Lock.PllLock     = LockStable;
    port->content.rx->Lock.EqLock      = LockStable;
    /* 2.2 Timing Lock Detection */
    LockStable = 0;
    if(port->content.lvrx->Lock == 1)
    {
        GSV2K11_INT_get_PAR_VS_LOCK_RAW_ST(port, &value);
        if(value == 1)
        {
            GSV2K11_INT_get_PAR_VS_LOCK_INT_ST(port, &value);
            if(value == 1)
                LockStable = 1;
            else
            {
                GSV2K11_INT_set_PAR_VS_LOCK_RAW_CLEAR(port, 1);
                GSV2K11_INT_set_PAR_VS_LOCK_RAW_CLEAR(port, 0);
            }
        }
    }
    /* 2.3 x7 mode protection for Vsync */
    Offset = port->content.lvrx->Config * 3;
    if((ParallelConfigTable[Offset+0] & 0x07) == 0x07)
    {
        /* 2.3.1 H lock detection for x7 mode */
        if(port->content.lvrx->Lock == 1)
        {
            GSV2K11_PAR_get_RB_RX_PAR_HS_LOCK(port, &LockStable);
        }
        /* 2.3.2 Trigger manual EQ lock */
        AvHalI2cReadField8(GSV2K11_PAR_MAP_ADDR(port),0x27,0xF0,4,&value);
        if(port->content.lvrx->Lock == 0)
        {
            AvHalI2cWriteField8(GSV2K11_PAR_MAP_ADDR(port),0x27,0xF0,0,0xC0);
        }
        else if((value & 0x08) == 0x00)
        {
            port->content.rx->HpdDelayExpire = 0;
            port->content.rx->EQDelayExpire = 0;
            AvHalI2cWriteField8(GSV2K11_PAR_MAP_ADDR(port),0x27,0xF0,0,0xC0);
            AvHalI2cWriteField8(GSV2K11_PAR_MAP_ADDR(port),0x1A,0xFF,0,0xFF);
            AvHalI2cWriteField8(GSV2K11_PAR_MAP_ADDR(port),0x1B,0xFF,0,0xFF);
        }
        /* 2.3.3 Trigger EQ delay */
        else if(LockStable == 1)
        {
            if(port->content.rx->EQDelayExpire > RxEQDelayExpireThreshold)
                LockStable = 1;
            else
        ...
        {
            port->content.rx->HpdDelayExpire = port->content.rx->HpdDelayExpire + 1;
            if((port->content.rx->HpdDelayExpire > 100) ||
               (port->content.rx->IsFreeRun != LockStable))
            {
                port->content.rx->HpdDelayExpire = 0;
                port->content.rx->EQDelayExpire = 0;
                value = 0x80 | ((((value&0x07)+1)&0x07)<<4);
                AvHalI2cWriteField8(GSV2K11_PAR_MAP_ADDR(port),0x27,0xF0,0,value);
                AvUapiOutputDebugFsm("Port %d: x7 EQ = %x",port->index, value);
            }
        }
    }
    if(port->content.rx->IsFreeRun != LockStable)
    {
        AvUapiOutputDebugFsm("Port %d: Timing Lock State = %d",port->index, LockStable);
    }
    port->content.rx->IsFreeRun        = LockStable;
    port->content.rx->IsInputStable    = LockStable;
    port->content.rx->Lock.DeRegenLock = LockStable;
    port->content.rx->Lock.VSyncLock   = LockStable;
