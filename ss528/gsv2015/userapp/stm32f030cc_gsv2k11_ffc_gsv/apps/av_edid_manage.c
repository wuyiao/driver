...
static const uint8 AvEdidIDManufacturerName[] =  {'G','S','V'};
static const uint8 AvEdidProductName[] =  {'G','S','V','2','K','1','1',' ',' ',' ',' '};
...
static const uint8 EdidCeaDolbyVision[] =  {0x46,0xD0,0x00,0x45,0x0B,0x90,0x86,0x60,0x76,0x8F};
...
/* EDID Function Declare Start */
    ...
    uint16 AvEdidFuncFindCeabTag(uint8 *EdidContent,uint8 TagNumber,uint8 SubVersion)
    {
        ...
        while(offset <= 0xfd)
        {
            ...
            else if((bTag == 7) && (TagNumber == 3) && (SubVersion == 1) &&
                 (EdidContent[offset+1] == 0x79) &&
                 (EdidContent[offset+2] == 0x00) &&
                 (EdidContent[offset+3] == 0x00))
            {
                CeabParam = (offset<<8) + bLen;
                return CeabParam;
            }
    ...
    uint8 AvEdidFuncCeabAudio(AvEdidReg *EdidReg,uint8 *InEdid,uint16 InCeabParam,uint8 *SinkEdid,uint16 SinkCeabParam,uint8 *OutEdid,uint8 OutEdidOffset)
    {
        ...
        EdidReg->EdidCeaAudioByte3[9]     = 0x03;
        EdidReg->EdidCeaAudioByte3[10]    = 0x00;
        EdidReg->EdidCeaAudioByte3[11]    = 0x03;
